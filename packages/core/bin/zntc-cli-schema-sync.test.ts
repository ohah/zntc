/**
 * zntc CLI flag ↔ `BuildOptions` ↔ WASM `BundleOptionsInput` schema sync 검증.
 *
 * #2112 의 schema sync 는 `BuildOptions` ↔ `KNOWN_CONFIG_KEYS` ↔ Zig DTO 만 검증했다.
 * 다음 두 갭이 남아 있어 silent 회귀 가능:
 *  1. CLI flag (`zntc.mjs`) ↔ `BuildOptions` 매핑 — 새 BuildOptions 키 추가 후 CLI flag
...
const CLI_PATH = join(__dirname, 'zntc.mjs');
...
/**
 * zntc.mjs 의 `parseArgs` 함수 안 `opts = { ... }` 객체 리터럴에서 키 추출.
 ...
 * `--host`, `--proxy`) 은 zntc.mjs source 에서 정규식 추출.
 */
function extractCliFlags(source: string): string[] {
  ...
  if (start < 0) throw new Error('parseArgs not found in zntc.mjs');
...
describe('CLI flag ↔ BuildOptions / TranspileOptions schema sync', () => {
  ...
  const cliOnlyFlags: ReadonlySet<string> = new Set([
    ...
    // CLI 의 standard flag 들. zntc 내부 BuildOptions 와 1:1 매핑되지 않고 caller
    ...
    // `charsetUtf8: boolean` (1:N 매핑). zntc.mjs 에서 enum→boolean 변환.
    '--charset=',
    ...
    if (missing.length > 0) {
      throw new Error(
        `[schema drift] BuildOptions/TranspileOptions 키가 CLI flag 로 노출 안 됨: ${missing.sort().join(', ')}\n` +
          `CLI flag 추가 (zntc.mjs parseArgs) OR buildOptionsOnlyKeys allowlist 에 등록.`,
      );
    }
  });
