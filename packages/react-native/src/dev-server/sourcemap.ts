// zntc sourcemap 후처리 — DevTools 의 x_google_ignoreList 확장 + production
// bundle 의 sourceRoot / 절대 경로 옵션. zntc 가 emit 한 기존 ignore hint 항목
// 보존 + node_modules + zntc internal source (zntc: prefix — runtime polyfill /
// babel transform 등) 추가. 잘못된 JSON 이면 rawJson 반환 (caller 가 빈 응답 회피).

import { isAbsolute, resolve } from 'node:path';

/**
 * NAPI 가 emit 한 zntc internal source 는 Rolldown virtual module convention
 * 으로 NUL prefix (U+0000) — 예: NUL + 'zntc:runtime/spread-array'.
 ...
 * URI scheme (`zntc:` / `http:` / `bun:` 등) prefix 가 있으면 virtual.
 ...
 *
 * `zntc:` prefix 는 NAPI emitter 의 internal source naming 과 sync — 변경 시
 * zntc core (Zig 측 emitter) 와 함께 업데이트 필요.
 */
...
export function postProcessSourceMap(rawJson: string, pathOpts?: SourcemapPathOptions): string {
  try {
    ...
    // zntc core 는 library/file output 을 위해 `file` 과 빈 `sourceRoot` 를 emit 한다.
