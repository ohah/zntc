...
// `@zntc/react-native` 의 dependency 로 자동 install (번개 parity) — 사용자
 ...
 *
 * `ZNTC_DEBUG_TERMINAL=1` 시 import 실패 reason 을 stderr 출력 — 사용자가
 * cli-server-api 미설치인지 다른 이유인지 추적. peer 미설치는 흔한 케이스라
 * default 는 silent.
 */
/**
 * caller projectRoot → @zntc/react-native → process.cwd 순으로 resolve 시도.
...
function resolveCliServerApiPath(projectRoot: string | undefined): string {
  ...
  // `@zntc/react-native` 자체 dependency — workspace install 시 자동 hoist.
  ...
  try {
    ...
    if (process.env.ZNTC_DEBUG_TERMINAL === '1') {
      process.stderr.write(
        `[zntc:rn-dev:debug] cli-server-api load failed: ${(err as Error)?.message ?? err}\n`,
      );
    }
    return null;
  }
