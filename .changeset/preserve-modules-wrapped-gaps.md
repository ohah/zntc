...
- **소비자 forwarding 이 심볼 갱신을 `init_X()` 호출 *전에*** 했다 → 값이 `__esm` 클로저 안에서 대입되므로 **여전히 undefined** 였다(init 은 memoize 라 재갱신 기회도 없다). **함수 선언만 hoisting 으로 살아남아** 테스트가 통과했고, 그 테스트는 **import 순서 덕에 provider 의 init 이 먼저 돌아** 버그를 가리고 있었다 — 내가 "제거했다" 고 주장한 바로 그 은폐 패턴이다. init 을 먼저 돌리고 그 다음 갱신하도록 고쳤다.
