...
ZNTC 의 웹 빌드 / dev server 검증용 예제 앱. styled-components 와 emotion 의 주요 패턴을 모아 1st-party transform 의 회귀 테스트 베이스.
...
`zntc.config.ts` 에서 `compiler.styledComponents` / `compiler.emotion` 활성. 두 transform 모두 1st-party 로 ZNTC 안에서 동작 (별도 babel/swc plugin 불필요).
...
| 사용자 명시 `.withConfig` MERGE | ✅ | 사용자 key 보존, ZNTC 누락만 prepend |
