 ...
 *
 * 기존 tests/integration/tests/vite-plugin.test.ts는 단일 파일 transform 단위 검증.
 * 본 테스트는 다중 TS/TSX 모듈 앱을 실제 `vite build`로 프로덕션 번들한 뒤 Playwright
 * 브라우저에서 로드해 동작까지 확인 — real-world E2E.
 *
 * Vite config에서 @zntc/vite-plugin(TS 소스)를 import하므로 Bun을 통해 실행해야 한다.
...
test.beforeAll(async () => {
  ...
  const pluginDir = join(PROJECT_ROOT, 'packages/vite-plugin');
...
test.describe('Vite app build (@zntc/vite-plugin) E2E', () => {
