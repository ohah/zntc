...
/**
 * Multi-module Vite 앱을 vite-plugin-zntc로 빌드 → 브라우저 실행 E2E.
 *
 * 기존 tests/integration/tests/vite-plugin-zntc.test.ts는 단일 파일 transform 단위 검증.
 * 본 테스트는 다중 TS/TSX 모듈 앱을 실제 `vite build`로 프로덕션 번들한 뒤 Playwright
 * 브라우저에서 로드해 동작까지 확인 — real-world E2E.
 *
 * Vite config에서 vite-plugin-zntc(TS 소스)를 import하므로 Bun을 통해 실행해야 한다.
...
<html>
  <head><meta charset="utf-8" /><title>vite+zntc e2e</title></head>
  <body>
    <div id="root"></div>
...
function makeViteConfig(pluginDir: string): string {
  return `import { zntc } from "${pluginDir.replace(/\\/g, '/')}/src/index.ts";
export default { plugins: [zntc()], build: { minify: true, sourcemap: true } };
`;
}
...
test.beforeAll(async () => {
  fixtureDir = await mkdtemp(join(tmpdir(), 'zntc-vite-e2e-'));
  ...
  const pluginDir = join(PROJECT_ROOT, 'packages/vite-plugin-zntc');
...
test.describe('Vite app build (vite-plugin-zntc) E2E', () => {
  test('다중 모듈 TS/TSX 앱이 번들되고 브라우저에서 정상 실행된다', async ({ page }) => {
    await page.goto(`http://localhost:${TEST_PORT}/`);
    await expect(page.getByTestId('greeting')).toHaveText('hello from ZNTC');
  });
