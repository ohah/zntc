...
describe('import attributes (with/assert) AST 보존', () => {
  test("기본 with { type: 'json' } 보존", async () => {
    ...
    try {
      const { stdout, exitCode } = await runZntcInDir(dir, ['entry.mjs']);
  ...
  test("중복 키는 에러 (with { type: 'json', type: 'webpack' })", async () => {
    ...
    try {
      const { stderr } = await runZntcInDir(dir, ['entry.mjs']);
  ...
  test('Babel fixture (string-literal): 파싱 통과', async () => {
    ...
    const { exitCode } = await runZntc([join(FIXTURES, 'import-attributes/string-literal.mjs')]);
    expect(exitCode).toBe(0);
  });

  test('Babel fixture (trailing-comma): 파싱 통과', async () => {
    const { exitCode } = await runZntc([join(FIXTURES, 'import-attributes/trailing-comma.mjs')]);
    expect(exitCode).toBe(0);
  });
});
...
describe('import defer/source (Stage 3) AST 보존', () => {
  ...
  test('Babel fixture (basic.mjs): 파싱 통과', async () => {
    const { exitCode } = await runZntc([join(FIXTURES, 'import-defer/basic.mjs')]);
    expect(exitCode).toBe(0);
  });
...
describe('TS type parameter modifier (const/in/out) AST 보존', () => {
  ...
  test('function<const T>', async () => {
    ...
    try {
      const { exitCode } = await runZntcInDir(dir, ['f.ts']);
      expect(exitCode).toBe(0);
    } finally {
      await cleanup();
    }
  ...
  test('Babel const-type-parameters fixture (모든 형태)', async () => {
    const { exitCode } = await runZntc([
      join(FIXTURES, 'ts-type-param-modifier/const-modifier.ts'),
    ]);
    expect(exitCode).toBe(0);
  });
