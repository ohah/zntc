...
describe.skipIf(!hasVite)('vite-plugin-zntc', () => {
  ...
  it('TypeScript transform → type annotations stripped', async () => {
    const fixture = await createFixture({
      'main.ts': `export const greeting: string = "hello from ZNTC";\nexport const num: number = 42;`,
    });
    ...
    expect(jsChunk.code).toContain('hello from ZNTC');
  ...
  it("RSC: 'use client' 디렉티브 보존 (Vite + ZNTC)", async () => {
    const fixture = await createFixture({
    ...
    const jsChunk = result.output.find((o: any) => o.type === 'chunk');
    expect(jsChunk).toBeDefined();
    // Vite/Rollup이 디렉티브를 출력 청크 top에 보존 (ZNTC transform이 디렉티브를 살린 상태로 넘김)
    expect(
  ...
  it("RSC: 'use server' 디렉티브 보존 (Vite + ZNTC)", async () => {
    const fixture = await createFixture({
