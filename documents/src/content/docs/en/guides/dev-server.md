---
title: Dev Server
description: SSE event stream and Control API in the ZNTC dev server
---

`zntc --serve --bundle <entry>` starts a dev server that exposes **2 interfaces for external automation/observability** in addition to standard HTTP/HMR.

| Endpoint | Purpose |
|---|---|
| `/sse/events` | Server-Sent Events stream — real-time build/watch events |
| `/reset-cache` | Control API — invalidate cache externally |
...
| `cache_reset` | Cache invalidated (Control API) | `{type}` |
