...
```ts
import { configureProfile, profileReport, transpile } from "@zts/core";
...
- `server` — `DevServerOptions` (defaults for `zts dev` / `--serve`. CLI flags override)
...
- `devMode` — wrap modules with `__zts_register()` factory + inject HMR runtime
...
Sub-phase keys (when `profile: ["..."]` or `ZTS_PROFILE` env is active): `scan` / `parse` / `resolve` /
...
```ts
import { buildAppSync, prepareAppDevSync } from "@zts/core";
...
const dev = prepareAppDevSync({ root: ".", outdir: ".zts-dev" });
```
...
```ts
import { loadConfig, defineConfig } from "@zts/core";
...
```ts
import { defineWorkspace, loadWorkspace } from "@zts/core";
...
counterpart to `zts bench --phase=...` — same engine. See [Benchmarks](/zts/en/reference/benchmarks/)
for results.
