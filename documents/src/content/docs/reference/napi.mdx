...
- `server` — `DevServerOptions` (`zts dev` / `--serve` 기본값. CLI flag 가 우선)
...
- `devMode` — 모듈을 `__zts_register()` 팩토리 + HMR 런타임 주입
...
`@emotion/babel-plugin` 과 동일한 결과를 plugin 등록 없이 옵션만 켜서 얻는다.
...
Sub-phase (`profile: ["..."]` 또는 `ZTS_PROFILE` env 활성 시): `scan` / `parse` / `resolve` /
...
CLI `zts bench --phase=...` 의 NAPI 대응 — 같은 engine 사용. 결과는 [Benchmarks](/zts/reference/benchmarks/) 참고.
