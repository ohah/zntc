...
| `--emit-disk-sourcemap[=false]` / `--no-emit-disk-sourcemap` | watch 모드에서 `.map` 을 디스크에 쓸지 (기본 `true`). dev 서버처럼 메모리로만 들고 갈 때 끔 |
...
| `--tree-shaking[=false]` / `--no-tree-shaking` | 트리쉐이킹 (기본 `true`). 디버깅·dead-code 보존이 필요할 때만 끔                                                       |
| `--scope-hoist[=false]` / `--no-scope-hoist`   | 스코프 호이스팅 (기본 `true`). 끄면 모듈별 wrapper 유지 — 출력 비교/디버깅용                                            |
| `--min-chunk-size=<n>`             | n 바이트 미만의 작은 공통 청크를 병합 (코드 스플리팅 시 청크 파편화 억제)                                                            |
...
| `--fallback:FROM=TO`                    | 정상 resolve 가 실패할 때만 `FROM` → `TO` 로 매핑 (`=false` → 빈 모듈). `--alias` 와 달리 우선 시도 후 폴백 |
| `--resolve-symlink-siblings`            | lookup 실패 시 source 디렉토리 realpath 에서 재시도 (RN/pnpm peer sibling 해석) |
| `--disable-hierarchical-lookup`         | 상위 `node_modules` 계층 탐색 비활성 (Metro 스타일 단일 트리 해석) |
...
**Dev Server 외부 인터페이스:** `/sse/events` (SSE 빌드 이벤트), `/reset-cache` (Control API).
...
| `--no-config`                | `zntc.config.*` 자동 탐색·로드를 건너뜀 — CLI 플래그만으로 동작 (config 영향 격리) |
| `--color` / `--no-color`     | 색 출력 강제/비활성 (`NO_COLOR` 환경변수 존중)                                     |
