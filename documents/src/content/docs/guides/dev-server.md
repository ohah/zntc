---
title: Dev Server
description: ZNTC dev server의 SSE 이벤트 스트림과 Control API
---

`zntc --serve --bundle <entry>`로 시작하는 dev 서버는 일반 HTTP/HMR 외에 **외부 자동화/관측 도구를 위한 2가지 인터페이스**를 제공합니다.

| 엔드포인트 | 용도 |
|---|---|
| `/sse/events` | Server-Sent Events 스트림 — 실시간 빌드/watch 이벤트 |
| `/reset-cache` | Control API — 외부에서 캐시 무효화 |
...
| `cache_reset` | 캐시 초기화 (Control API) | `{type}` |
