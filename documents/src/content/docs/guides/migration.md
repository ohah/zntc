---
title: 마이그레이션 가이드
description: esbuild, Vite, webpack에서 ZNTC로 마이그레이션하는 방법을 알아봅니다.
---
...
| esbuild                         | ZNTC                                           | 비고                                                          |
| ------------------------------- | --------------------------------------------- | ------------------------------------------------------------- |
| `esbuild src/index.ts --bundle` | `zntc --bundle src/index.ts`                   | 동일                                                          |
...
| `--charset=ascii`               | `--ascii-only`                                | ZNTC 는 전용 플래그. 비-ASCII를 `\uXXXX` 이스케이프            |
...
| `--mangle-quoted`           | 미지원                                                        |
| `--analyze` (tree 포맷)     | `--analyze` JSON + [/analyze/](/zntc/analyze/) 시각화. CLI tree 포맷은 예정 |
...
함수형 config(`defineConfig(({ command, mode }) => ...)`)에서 Vite 는 `command` 가 `"build" | "serve"` 두 가지인 반면, ZNTC 는 `"bundle" | "serve" | "watch"` 세 가지입니다. Vite 에서 `command === "build"` 분기를 쓰던 코드는 ZNTC 에서 `"bundle"` 로 바꿔야 하며, watch 모드(`zntc --watch`)는 별도 값으로 들어옵니다.
:::
...
| `vite build`                         | `zntc build` 또는 library build는 `zntc --bundle <entry> --outdir dist --splitting --minify --sourcemap` |
...
| PostCSS (`postcss.config.js`)        | 앱 모드에서 지원. `zntc dev`는 PostCSS dependency watch와 CSS-only HMR 지원                             |
...
| webpack 로더                                    | ZNTC 대응                                                     |
| ----------------------------------------------- | ------------------------------------------------------------ |
| `ts-loader` / `babel-loader`                    | 불필요. ZNTC가 TS/JSX 직접 처리                               |
| `@swc/swc-loader` / `esbuild-loader`            | 불필요. ZNTC가 대체                                           |
...
| `webpack.config.js` 함수형/멀티 컨피그                 | 미지원. `zntc.config.ts` 단일 export                                                    |
