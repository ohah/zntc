...
#### Chrome CDP 번들 검증 CLI

내부 테스트는 Chrome DevTools Protocol 로 번들을 실제 브라우저에서 실행해 sourcemap·런타임 에러를 검증합니다. 이 경로를 사용자 CLI 명령(`zntc verify --browser`)으로 승격해, 빌드 결과를 헤드리스 Chrome 에서 실행하고 console error·uncaught·sourcemap 해석 결과를 종료 코드 + JSON 으로 리포트합니다 (Playwright 는 optional dependency).
