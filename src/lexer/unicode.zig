...
//! - ID_Continue: 식별자 계속 문자 (ID_Start + Mn, Mc, Nd, Pc + Other_ID_Continue + ZWNJ + ZWJ)
//!
//! UTF-8 디코딩 유틸리티도 포함.
//!
//! 테이블은 Unicode 16.0.0 기준으로 생성되었다.
//! Python unicodedata (13.0.0) + Unicode 14.0.0/15.0.0/15.1.0/16.0.0 보충 범위.
//! ECMAScript 스펙에 따라 Pattern_Syntax 문자(U+2E2F)는 제외.
//!
...
// ============================================================
// 내부 구현: 이진 탐색 기반 범위 테이블 조회
// ============================================================

/// 정렬된 [start, end] 범위 테이블에서 cp가 포함되는지 이진 탐색으로 확인한다.
/// 테이블은 comptime에 정렬되어 있어야 한다 (start 기준 오름차순, 범위 겹침 없음).
fn binarySearchRanges(comptime table: []const [2]u21, cp: u21) bool {
    var low: usize = 0;
    ...
    while (low < high) {
        ...
        if (cp < range[0]) {
            ...
            return true; // range[0] <= cp <= range[1]
        }
    }
    return false;
}
...
/// Unicode ID_Start 판별.
/// 675개 범위를 이진 탐색으로 O(log n) 조회.
fn isUnicodeIdStart(cp: u21) bool {
    return binarySearchRanges(&id_start_table, cp);
}

/// Unicode ID_Continue 판별.
/// ID_Start에 포함되면 true, 아니면 ID_Continue 전용 테이블(390개 범위)을 추가 조회.
fn isUnicodeIdContinue(cp: u21) bool {
    if (binarySearchRanges(&id_start_table, cp)) return true;
    return binarySearchRanges(&id_continue_only_table, cp);
}

// ============================================================
// Unicode ID_Start 범위 테이블 (비ASCII, >= 0x80)
// Unicode 16.0.0 기준, 675개 범위.
// Pattern_Syntax 문자(U+2E2F VERTICAL TILDE) 제외.
// ============================================================
const id_start_table = [_][2]u21{
    .{ 0x00AA, 0x00AA },
    .{ 0x00B5, 0x00B5 },
    .{ 0x00BA, 0x00BA },
    .{ 0x00C0, 0x00D6 },
    ...
    .{ 0x2DD0, 0x2DD6 },
    .{ 0x2DD8, 0x2DDE },
    // U+2E2F (VERTICAL TILDE) 제외 — Pattern_Syntax
    .{ 0x3005, 0x3007 },
    .{ 0x3021, 0x3029 },
    .{ 0x3031, 0x3035 },
    .{ 0x3038, 0x303C },
...
// ============================================================
// Unicode ID_Continue 전용 범위 테이블 (비ASCII, >= 0x80)
// ID_Start에 포함되지 않는 ID_Continue 문자만 담는다.
// Mn (Nonspacing_Mark), Mc (Spacing_Mark), Nd (Decimal_Number),
// Pc (Connector_Punctuation), Other_ID_Continue.
// Unicode 16.0.0 기준, 390개 범위.
// ============================================================
const id_continue_only_table = [_][2]u21{
    .{ 0x00B7, 0x00B7 },
    .{ 0x0300, 0x036F },
    .{ 0x0387, 0x0387 },
    .{ 0x0483, 0x0487 },
