...
const type_members = @import("ts/type_members.zig");

pub const PropertySignatureFlags = type_members.PropertySignatureFlags;
pub const parseIndexSignature = type_members.parseIndexSignature;
...
pub fn parseTsInterfaceDeclaration(self: *Parser) ParseError2!NodeIndex {
    ...
    // interface body
    const body = try type_members.parseObjectType(self);
...
fn parsePrimaryType(self: *Parser) ParseError2!NodeIndex {
    ...
    switch (self.current()) {
        ...
        .l_curly => {
            ...
            return type_members.parseObjectType(self);
        },
        // 제네릭 함수 타입: <T>(x: T) => R
        .l_angle => {
            const type_params = try parseTsTypeParameterDeclaration(self);
            try self.expect(.l_paren);
            const params = try type_members.parseTypeMemberParamList(self);
    ...
    if (self.current() == .comma) {
        ...
        while (try self.eat(.comma)) {
            ...
            if (self.current() == .dot3 or isFunctionTypeParam(self)) {
                const param = try type_members.parseTypeMemberParam(self);
                try self.scratch.append(self.allocator, param);
