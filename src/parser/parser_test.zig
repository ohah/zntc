...
test "Parser regression #4515: CoverInitializedName 의 key 는 IdentifierReference 뿐" {
    // `{[k] = 1}` / `{"s" = 1}` / `{1 = 1}` 은 대입할 바인딩 이름이 없어 어느 문맥에서도
    // valid 가 아니다 (V8 도 SyntaxError). 수정 전에는 파서가 조용히 수용하고 codegen 이
    // `({ [k]:[k] =1 } = o)` 같은 invalid JS 를 방출했다 — accept-invalid.
    // bare shorthand(`{ "s" }`)가 이미 같은 키 종류를 거부하는 것과 대칭.
    try expectParseError(
        \\const k = "a"; const o = {}; ({ [k] = 1 } = o);
    , .{ .message_contains = "Invalid shorthand property initializer" });
    try expectParseError(
        \\const o = {}; ({ "s" = 1 } = o);
    , .{ .message_contains = "Invalid shorthand property initializer" });
    try expectParseError(
        \\const o = {}; ({ 1 = 1 } = o);
    , .{ .message_contains = "Invalid shorthand property initializer" });
    // arrow 파라미터 자리도 거부된다. 단 이쪽은 object-literal cover 경로가 아니라
    // binding pattern 파서(binding.zig)를 타므로 진단 문구가 다르다 (`Expected ':'`) —
    // 원래부터 막혀 있던 경로다. 여기서는 "거부된다"는 사실만 고정한다.
    try expectParseError(
        \\const f = ({ [k] = 1 }) => k;
    , .{});
}

test "Parser regression #4515: 정상 CoverInitializedName 은 그대로 통과 (회귀 0)" {
    // identifier key + 기본값 = 합법. 구조분해로 소비되기만 하면 된다.
    try expectNoParseError(
        \\const o = {}; let x; ({ x = 1 } = o);
    );
    try expectNoParseError(
    ...
    // computed / string key 는 **콜론과 함께** 쓰면 당연히 합법.
    try expectNoParseError(
        \\const k = "a"; const o = {}; let t; ({ [k]: t = 1 } = o);
    );
    try expectNoParseError(
        \\const o = {}; let t; ({ "s": t = 1 } = o);
    );
}
