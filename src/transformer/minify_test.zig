...
// #3559: top-level primitive const 가 (a) constant-fold 표현식에서 여러 번 쓰이고
// (b) object shorthand property `{ A }` key 로도 쓰이는 경우. inline 패스는 (a) 의
// 비-forbidden ref 를 리터럴로 치환하며 ref_deltas 를 올리지만 shorthand-key ref 는
// 의도적으로 보존한다(replace 시 `{[...]}` 구문 깨짐). effectiveRefCount 가 base −
// delta 누적이라 0 으로 포화돼도, 살아있는 shorthand-key ref 가 있으면 declaration 을
// 제거하면 안 된다 (제거 시 `A:A` 의 RHS 가 ReferenceError). 회귀 가드.
test "dead store: shorthand-key ref 가 살아있으면 const declaration 보존 (#3559)" {
    try expectMinifyTopLevelInline(
        "const A = \"x\"; const B = \"y\"; const O = { A, B, C: A + A + B }; globalThis.sink = O;",
        "const A=\"x\",B=\"y\",O={A,B,C:\"xxy\"};globalThis.sink=O;",
    );
}
