...
test "#4394 emotion (sourceMap): 보간 템플릿에 label + sourceMap 둘 다 독립 적용" {
    // 보간 css 의 첫 quasi(label)와 마지막 quasi(sourceMap) 재작성은 독립적이다.
    // 한쪽 재작성이 (예상치 못한 quasi 모양으로) 건너뛰어도 다른 쪽은 적용돼야 한다.
    var r = try e2eFull(
        std.testing.allocator,
        \\import { css } from "@emotion/react";
        \\const themed = css`color: ${color}; padding: 8px;`;
    ...
    defer r.deinit();
    try expectAutoLabel(r.output, "themed"); // 첫 quasi: label
    try expectSourceMapComment(r.output); // 마지막 quasi: sourceMap
    try std.testing.expect(std.mem.indexOf(u8, r.output, "${color}") != null);
}
