...
test "dispatchMcpRequest: tools/list → 4개 tool 명세 포함" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    const dir_path = try tmp.dir.realpathAlloc(std.testing.allocator, ".");
    defer std.testing.allocator.free(dir_path);
    ...
    // tool drop 회귀 감지용 — 미래에 ping_app 제거 시 명시 fail.
    try std.testing.expect(std.mem.indexOf(u8, resp.items, "\"ping_app\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, resp.items, "\"error\"") == null);
}
