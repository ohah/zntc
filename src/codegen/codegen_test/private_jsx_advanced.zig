...
test "#3680-V8: extends getBase() 의 side effect 가 super.x access 마다 중복 안 됨 (es2021)" {
    ...
    defer r.deinit();
    try std.testing.expect(std.mem.indexOf(u8, r.output, "function _m_fn") != null);
    // _m_fn body 만 추출 (다음 class 또는 다음 function 까지)
    const fn_start = std.mem.indexOf(u8, r.output, "function _m_fn") orelse unreachable;
    const after = r.output[fn_start + 10 ..];
    const next_class = std.mem.indexOf(u8, after, "class ") orelse after.len;
    const next_fn = std.mem.indexOf(u8, after, "function ") orelse after.len;
    const fn_body = after[0..@min(next_class, next_fn)];
    // _m_fn body 안에 `getBase()` 가 inline 되면 부작용 중복 (회귀).
    try std.testing.expect(std.mem.indexOf(u8, fn_body, "getBase()") == null);
    // raw `D.prototype` 도 fn_body 에 없어야 (VSHADOW: shadow 차단).
    try std.testing.expect(std.mem.indexOf(u8, fn_body, "D.prototype") == null);
    // alias var `var _a = globalThis.Object.getPrototypeOf(D.prototype);` 가 class 뒤에 emit.
    try std.testing.expect(std.mem.indexOf(u8, r.output, "globalThis.Object.getPrototypeOf(D.prototype)") != null);
}

// === post-review 3rd-round 정밀 fix 후속: code-review max 가 surface 한 6건 ===

// VOPT: optional chain super (es2020 path) 가 via_proto_chain flag 미반영.
// `super?.foo?.()` 가 raw `D.prototype.foo` 로 lowering 되어 D 가 method shadow 시 wrong dispatch.
test "#3680-VOPT: optional chain super 가 via_proto_chain 사용 (es2019)" {
    var r = try e2eFull(std.testing.allocator,
        \\function getBase(){ return class { foo(){return "B"} } }
        \\class D extends getBase() {
        \\  foo() { return "D" }
        \\  #m() { return super.foo?.() }
        \\  run() { return this.#m() }
        \\}
    , .{ .unsupported = helpers.compat.fromESTarget(.es2019) }, .{ .minify_whitespace = true, .assert_no_raw_private_syntax = true }, ".ts");
    defer r.deinit();
    // _m_fn body 안의 super-prop base 는 D.prototype 가 아니라 Object.getPrototypeOf(D.prototype).
    const fn_start = std.mem.indexOf(u8, r.output, "function _m_fn") orelse unreachable;
    const fn_body = r.output[fn_start..@min(r.output.len, fn_start + 400)];
    // raw `D.prototype.foo` (Object.getPrototypeOf 없이) 가 inline 되면 회귀.
    try std.testing.expect(std.mem.indexOf(u8, fn_body, "Object.getPrototypeOf(D.prototype)") != null);
}

// VSHADOW: V8 fix 가 emit 한 D 식별자가 method body 안 local 변수 (let D = ...) 에 shadow.
// outer alias hoist 로 unique 이름 사용해야 함.
test "#3680-VSHADOW: V8 fix 의 class ref 가 method body local shadow 영향 안 받음 (es2021)" {
    var r = try e2eTarget(std.testing.allocator,
        \\class D extends class { foo(){return "B"} } {
        \\  #m() { let D = 1; return super.foo(); }
        \\  run() { return this.#m() }
        \\}
    , .es2021);
    defer r.deinit();
    // _m_fn body 만 (다음 `class` 또는 다음 `function` 까지) 추출.
    const fn_start = std.mem.indexOf(u8, r.output, "function _m_fn") orelse unreachable;
    const after = r.output[fn_start + 10 ..];
    const next_class = std.mem.indexOf(u8, after, "class ") orelse after.len;
    const next_fn = std.mem.indexOf(u8, after, "function ") orelse after.len;
    const fn_body = after[0..@min(next_class, next_fn)];
    // body 안에 `let D=` 가 있음 확인 (회귀 setup).
    try std.testing.expect(std.mem.indexOf(u8, fn_body, "let D=") != null);
    // raw `D.prototype` 이 body 안에서 reference 되면 shadow 영향. outer alias 또는 globalThis 만 OK.
    try std.testing.expect(std.mem.indexOf(u8, fn_body, "D.prototype") == null);
}

// VEXPR: named class_expression + static private field with super → IIFE 안 descriptor TDZ.
// descriptor 가 IIFE 안 class_decl 뒤에 emit 돼야 함.
test "#3680-VEXPR: named class_expression static private init super TDZ 회피 (es2021)" {
    var r = try e2eTarget(std.testing.allocator,
        \\class Base { static factor(){return "ok"} }
        \\let X = class D extends Base {
        \\  static #x = super.factor();
        \\  static get(){ return D.#x }
        \\};
    , .es2021);
...
// VASSIGN: assign-semantics path (experimentalDecorators) 의 V1 fix 도 descriptor 를 class 뒤로.
test "#3680-VASSIGN: assign-semantics static private init super TDZ 회피 (es2021)" {
    var r = try e2eFull(std.testing.allocator,
        \\class Base { static factor(){return "ok"} }
        \\class D extends Base {
        \\  static #x = super.factor();
...
// VANON: anonymous class_expression with static private + non-id extends 의 V8 fix 활성화.
test "#3680-VANON: anonymous class_expression V8 fix 활성화 (es2021)" {
    var r = try e2eTarget(std.testing.allocator,
        \\function getBase(){ return class { foo(){return "X"} } }
        \\let X = class extends getBase() {
        \\  static #y = 1;
        \\  #m(){ return super.foo() + super.foo() }
        \\  run(){ return this.#m() }
        \\};
    ...
    const next_fn = std.mem.indexOf(u8, after, "function ") orelse after.len;
    const fn_body = after[0..@min(next_class, next_fn)];
    // _m_fn body 안에 raw `getBase()` 가 inline 되면 V8 fix 비활성 (회귀).
    try std.testing.expect(std.mem.indexOf(u8, fn_body, "getBase()") == null);
    // alias var 가 IIFE 안에 emit (Object.getPrototypeOf 가 출력 어딘가에 있어야).
    try std.testing.expect(std.mem.indexOf(u8, r.output, "globalThis.Object.getPrototypeOf") != null);
}

// VSB: static block + super at es2021 target (class_static_block lowering) → super raw leak.
test "#3680-VSB: static block 안 super 가 es2021 target 에서 lowering" {
    var r = try e2eTarget(std.testing.allocator,
        \\class Base { static x = 1 }
        \\class D extends Base {
        \\  static { globalThis.__sb_result = super.x; }
        \\}
    , .es2021);
    defer r.deinit();
    // IIFE 안에 raw `super.x` 가 leak 되면 SyntaxError.
    try std.testing.expect(std.mem.indexOf(u8, r.output, "super.x") == null);
}
