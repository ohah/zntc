...
// ---- #4481 code-review max 지적 반영분 ----
//
// (return/throw 직후 *괄호 안* 주석의 ASI 회귀는 이 하네스가 `cg.comments` 를 배선하지
//  않아 재현되지 않는다 → transpile.zig 의 #4481 테스트가 커버.)

test "#4481 minify: 조건식이 식별자 `let` 으로 시작하면 폴딩하지 않는다" {
    // sloppy `var let = [...]`. `let` 은 식별자라 precedence 괄호가 안 붙는데, statement 가
    // `let [` 로 시작하면 파서가 lexical 선언(배열 구조분해)으로 읽는다 → SyntaxError.
    // 폴딩을 포기하면 `if(` 가 남아 statement 선두가 아니게 되므로 안전하다.
    // `var let` 은 sloppy mode 전용 → `.js` 확장자로 파싱해야 한다(.ts 는 strict).
    var r = try helpers.e2eFull(std.testing.allocator,
        \\var let = [1]; function g() { return 1; } if (let[0]) g();
    , .{}, .{ .minify_whitespace = true, .minify_syntax = true }, ".js");
    defer r.deinit();
    try std.testing.expect(std.mem.indexOf(u8, r.output, "if(let[0])g()") != null);
}

test "#4481 minify: if/else-return 체인 ternary 도 test 괄호 유지" {
    // tryEmitReturnFallthrough(else 없음)와 달리 emitIfReturnChainTail 을 타는 경로.
    // 괄호가 빠지면 `return m=g()?h(m):0` = `return m = (g()?h(m):0)` 로 의미가 바뀐다.
    var r = try e2eMinify(std.testing.allocator,
        \\function f(g, h) { let m; if ((m = g())) { return h(m); } else { return 0; } }
    );
    defer r.deinit();
    try std.testing.expect(std.mem.indexOf(u8, r.output, "return(m=g())?h(m):0") != null);
}

test "#4481 minify: `??` 조건은 `&&` 폴딩 시 괄호 필수 (혼용 금지)" {
    // ES 는 `a??b&&c` 처럼 `??` 와 `&&` 의 괄호 없는 혼용을 금지한다 → SyntaxError.
    // `&&` 폴딩의 좌측 level(.logical_and.lower() = .logical_or)이 이 괄호를 재유도한다.
    var r = try e2eMinify(std.testing.allocator,
        \\function f(a, b, c) { if ((a ?? b)) c(); }
    );
    defer r.deinit();
    try std.testing.expect(std.mem.indexOf(u8, r.output, "(a??b)&&c()") != null);
    // 반면 `?:` 폴딩의 test level(.conditional)은 `??` 보다 느슨해 괄호가 필요없다.
    var r2 = try e2eMinify(std.testing.allocator,
        \\function f(a, b, c, d) { if ((a ?? b)) c(); else d(); }
    );
    defer r2.deinit();
    try std.testing.expect(std.mem.indexOf(u8, r2.output, "a??b?c():d()") != null);
}

test "#4481 minify: 폴딩된 return 은 test 가 괄호로 시작하면 공백을 낭비하지 않는다" {
    var r = try e2eMinify(std.testing.allocator,
        \\function f(g) { let m; if ((m = g())) return 1; return 2; }
        \\function h(c) { if (!c) return 1; return 2; }
    );
    defer r.deinit();
