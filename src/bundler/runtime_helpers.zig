...
pub const HMR_RUNTIME =
    ...
    \\      // eval만 실행 — $RefreshReg$가 새 컴포넌트를 등록.
    \\      // reset()/fn()은 불필요: IIFE 스코프와 eval 글로벌 스코프가 다르므로
    \\      // 모듈 변수 교체 불가. React Refresh가 컴포넌트 리렌더링을 처리.
