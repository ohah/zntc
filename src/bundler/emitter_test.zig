...
// PR-3b-i (lazy compilation): emitChunks 의 restrict_to_chunk 로 *단일* 청크만 emit.
// dev lazy 의 on-demand 컴파일 전제 — 첫 GET 시 해당 lazy 청크만 빌드. lazy seed 청크는
// 정상 emit 에서 skip 되지만 restrict 면 force-emit(미파싱→파싱된 on-demand 청크 경로).
test "PR-3b-i: emitChunks restrict_to_chunk — 단일 청크 emit + lazy seed force" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    const dp = try tmp.dir.realPathFileAlloc(std.testing.io, ".", std.testing.allocator);
    defer std.testing.allocator.free(dp);
    const ep = try std.fs.path.resolve(std.testing.allocator, &.{ dp, "entry.ts" });
    ...
    var graph = ModuleGraph.init(std.testing.allocator, &cache);
    defer graph.deinit();
    // dev + splitting + lazy → heavy 가 미파싱 lazy seed.
    graph.dev_mode = true;
    graph.code_splitting = true;
    graph.lazy_compilation = true;
    try graph.build(std.testing.io, &.{ep});

    var cg = try chunk_mod.generateChunks(std.testing.allocator, &graph, &.{ep}, .{});
    defer cg.deinit();

    const opts: EmitOptions = .{ .format = .iife, .code_splitting = true, .dev_mode = true };

    // 전체 emit: lazy heavy 청크 skip → entry 청크만.
    const full = try emitter.emitChunks(std.testing.allocator, &graph, &cg, &opts, null);
    defer {
        for (full) |o| o.deinit(std.testing.allocator);
        std.testing.allocator.free(full);
    }
    try std.testing.expectEqual(@as(usize, 1), full.len);
    try std.testing.expect(std.mem.indexOf(u8, full[0].contents, "HEAVY_BODY_MARKER") == null);
    try std.testing.expect(std.mem.indexOf(u8, full[0].contents, "__zntc_load_chunk") != null);

    // 청크 인덱스 식별 — entry 는 *정적* entry_point(비-dynamic), lazy 는 is_lazy_seed.
    // (last-wins 모호성 방지: 정적 entry_point 만 entry_idx 로.)
    var lazy_idx: ?usize = null;
    var entry_idx: ?usize = null;
    for (cg.chunks.items, 0..) |c, i| {
        if (c.is_lazy_seed) {
            lazy_idx = i;
        } else switch (c.kind) {
            .entry_point => |ep_info| if (!ep_info.is_dynamic and c.modules.items.len > 0) {
                entry_idx = i;
            },
            else => {},
        }
    }
    const li = lazy_idx orelse return error.TestUnexpectedResult;
    const ei = entry_idx orelse return error.TestUnexpectedResult;

    // (1) restrict = lazy 청크 → force-emit 1 output(heavy stub). entry 내용 없음.
    var lazy_opts = opts;
    lazy_opts.restrict_to_chunk = li;
    const only_lazy = try emitter.emitChunks(std.testing.allocator, &graph, &cg, &lazy_opts, null);
    defer {
        for (only_lazy) |o| o.deinit(std.testing.allocator);
        std.testing.allocator.free(only_lazy);
    }
    try std.testing.expectEqual(@as(usize, 1), only_lazy.len);
    try std.testing.expect(std.mem.indexOf(u8, only_lazy[0].contents, "heavy.js") != null); // heavy 모듈 register
    try std.testing.expect(std.mem.indexOf(u8, only_lazy[0].contents, "__zntc_load_chunk") == null); // entry 미emit
    // force-emit 된 lazy 청크는 미파싱이라 *stub* — heavy 본문은 없다(파싱은 PR-3b-iii).
    try std.testing.expect(std.mem.indexOf(u8, only_lazy[0].contents, "HEAVY_BODY_MARKER") == null);

    // (2) restrict = entry 청크 → full emit 의 entry 와 byte-identical (restrict 가 출력 안 바꿈).
    var entry_opts = opts;
    entry_opts.restrict_to_chunk = ei;
    const only_entry = try emitter.emitChunks(std.testing.allocator, &graph, &cg, &entry_opts, null);
    defer {
        for (only_entry) |o| o.deinit(std.testing.allocator);
        std.testing.allocator.free(only_entry);
    }
    try std.testing.expectEqual(@as(usize, 1), only_entry.len);
    try std.testing.expectEqualStrings(full[0].contents, only_entry[0].contents);
}
