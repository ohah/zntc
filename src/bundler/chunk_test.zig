...
test "generateChunks: single entry, no dynamic imports" {
    ...
    var cg = try chunk_mod.generateChunks(alloc, &modules, &.{"a.ts"}, null);
    defer cg.deinit();
...
test "generateChunks: dynamic import creates separate chunk" {
    ...
    var cg = try chunk_mod.generateChunks(alloc, &modules, &.{"index.ts"}, null);
    defer cg.deinit();
...
test "generateChunks: shared module creates common chunk" {
    ...
    var cg = try chunk_mod.generateChunks(alloc, &modules, &.{ "a.ts", "b.ts" }, null);
    defer cg.deinit();
...
test "generateChunks: diamond dependency" {
    ...
    var cg = try chunk_mod.generateChunks(alloc, &modules, &.{ "a.ts", "c.ts" }, null);
    defer cg.deinit();
...
test "generateChunks: no modules" {
    ...
    var cg = try chunk_mod.generateChunks(alloc, empty_modules, &.{"entry.ts"}, null);
    defer cg.deinit();
...
test "generateChunks: three entries sharing a module" {
    ...
    var cg = try chunk_mod.generateChunks(alloc, &modules, &.{ "a.ts", "b.ts", "c.ts" }, null);
    defer cg.deinit();
...
test "computeCrossChunkLinks: no cross-chunk deps — 모든 모듈이 같은 청크" {
    ...
    try chunk_mod.computeCrossChunkLinks(&cg, &modules, alloc, null);
