...
test "LazyDevSplitting: dev+split → 글로벌 __zntc_modules per-module 등록 + cross-chunk hot-replace (#4038 재해결, RFC_LAZY_DEV_MODULE_HMR PR-2)" {
    // #4038 의 *이상적* 재해결(RFC_LAZY_DEV_MODULE_HMR): dev+split 도 wrap-all 을 켜
    // 모듈을 개별 __esm/__commonJS factory 로 만들고, 글로벌(globalThis-backed)
    // `__zntc_modules` 레지스트리에 per-module 등록한다 → 청크 경계를 넘는
    // hot-replace 가능. cross-chunk *정적* 해석은 production __zntc_require 가 그대로
    // 담당(하이브리드 공존). 이전(PR-2 epic 전)엔 dev 레지스트리를 통째로 끄고
    // production wrapping 만 썼다(이 테스트의 옛 가드 = `__zntc_modules` 부재).
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    var has_hmr_registry = false; // 글로벌 __zntc_modules per-module 레지스트리 (PR-2 핵심)
    var has_apply_update = false; // hot-replace 머신러리 (entry 청크 1회)
    var has_global_registry = false; // globalThis-backed (청크 경계 공유의 핵심)
    for (outs) |o| {
        if (std.mem.indexOf(u8, o.contents, "__zntc_register") != null) has_register = true;
        if (std.mem.indexOf(u8, o.contents, "__zntc_load_chunk(\"") != null) has_loader = true;
        if (std.mem.indexOf(u8, o.contents, "__zntc_require(\"") != null) has_require_bootstrap = true;
        if (std.mem.indexOf(u8, o.contents, "__zntc_modules") != null) has_hmr_registry = true;
        if (std.mem.indexOf(u8, o.contents, "__zntc_apply_update") != null) has_apply_update = true;
        // 글로벌 백킹: `__zntc_g.__zntc_modules ||` 멱등 self-install 패턴 (청크마다
        // 같은 globalThis 객체를 공유 → cross-chunk lookup 성립).
        if (std.mem.indexOf(u8, o.contents, "__zntc_g.__zntc_modules") != null) has_global_registry = true;
        // 빈 dev_id 참조(__zntc_modules[""]) 절대 금지 — setDevId 로 모듈 ID 채움.
        try std.testing.expect(std.mem.indexOf(u8, o.contents, "[\"\"]") == null);
        // `export *`(re_export_star)가 cross-chunk export 로 새 invalid JS(`exports.* = *;`)
        // 가 되지 않는다 (emitLazyEntryExportAll re_export_star 스킵).
        try std.testing.expect(std.mem.indexOf(u8, o.contents, "exports.* =") == null);
        try std.testing.expect(std.mem.indexOf(u8, o.contents, "exports.*=") == null);
        // IIFE 청크에 ESM import/export 누출 금지.
        try std.testing.expect(std.mem.indexOf(u8, o.contents, "\nexport {") == null);
        try std.testing.expect(std.mem.indexOf(u8, o.contents, "\nimport {") == null);
    ...
    // PR-2: dev+split 도 글로벌 per-module HMR 레지스트리를 깐다(옛 가드 반전).
    try std.testing.expect(has_hmr_registry);
    try std.testing.expect(has_apply_update); // entry 청크에 hot-replace 머신러리 1회
    try std.testing.expect(has_global_registry); // globalThis-backed (cross-chunk 공유)
...
test "LazyDevSplitting: dev+split+collect_module_codes 가 청크 모듈 dev code 수집 (RFC_LAZY_DEV_MODULE_HMR PR-1)" {
    // PR-1 (인프라): dev+code_splitting+lazy 빌드에서도 emit 이 per-module HMR code 를
    // 수집해야 한다. 현재(main)는 dev_split 이면 emitChunks 경로로 가는데, 이 경로는
    // module_dev_codes 를 전혀 채우지 않아(emitChunks 가 []OutputFile 만 반환) onRebuild
    // 의 updates 가 항상 0 이다 (RFC §2 부수 버그). 이 테스트는 *초기* 빌드에서 entry
    // 청크에 포함된 모듈(entry/shared)의 dev code 가 수집됨을 가드한다.
    //   - 현재: result.module_dev_codes == null → red.
    //   - PR-1 구현 후: entry/shared 의 ModuleDevCode 가 수집됨 → green.
    // (동적 lazy 청크 모듈의 수집/교체는 force-parse materialize 후속 PR 범위.)
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "shared.ts", "export const s = 'SHARED_V1';");
    try writeFile(tmp.dir, "lazy.ts", "import { s } from './shared';\nexport function render() { return 'LAZY:' + s; }");
    try writeFile(tmp.dir, "entry.ts",
        \\import { s } from './shared';
        \\console.log('entry', s);
        \\async function go() { const m = await import('./lazy'); console.log(m.render()); }
        \\go();
    );
    const entry = try absPath(&tmp, "entry.ts");
    defer std.testing.allocator.free(entry);
    ...
    // dev+split 도 per-module dev code 를 노출해야 한다 (현재 null → red).
    const codes = result.module_dev_codes orelse return error.TestUnexpectedResult;

    // entry 청크에 정적으로 포함된 entry/shared 모듈의 dev code 가 수집됐는지.
    var saw_entry = false;
    var saw_shared = false;
    for (codes) |c| {
        try std.testing.expect(c.id.len > 0);
        try std.testing.expect(c.code.len > 0);
        if (std.mem.indexOf(u8, c.id, "entry.ts") != null) saw_entry = true;
        if (std.mem.indexOf(u8, c.id, "shared.ts") != null) saw_shared = true;
    }
    try std.testing.expect(saw_entry);
...
// PR-4-ii (재귀 lazy 전제 검증): force-parse 한 seed(heavy)가 *자기 자신의* 동적 import
// (deeper)를 가지면, 그 중첩 타겟은 force-parse 목록에 없으므로 **새 lazy seed 로 deferred**
// 되어 result.lazy_seed_paths 에 나와야 한다. dev server 가 이걸 누적(addSeedPaths)하면
// deeper 도 on-demand 라우트로 역참조된다. (없으면 재귀 lazy 가 bundler 변경 필요 → 큼.)
test "LazyCompilation PR-4-ii: force-parse 한 seed 의 중첩 import() 가 새 lazy seed 로 노출" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "deeper.ts", "export const d = 'DEEPER';");
    try writeFile(tmp.dir, "heavy.ts",
        \\export async function load() { const m = await import('./deeper'); return m.d; }
    );
    try writeFile(tmp.dir, "entry.ts",
        \\async function go() { const m = await import('./heavy'); console.log(await m.load()); }
        \\go();
    );
    const entry = try absPath(&tmp, "entry.ts");
    defer std.testing.allocator.free(entry);
    const heavy_abs = try absPath(&tmp, "heavy.ts");
    defer std.testing.allocator.free(heavy_abs);
    const deeper_abs = try absPath(&tmp, "deeper.ts");
    defer std.testing.allocator.free(deeper_abs);
    ...
    var bnd = Bundler.init(std.testing.allocator, .{
        ...
        .code_splitting = true,
        .lazy_compilation = true,
        .lazy_force_parse = &.{heavy_abs}, // heavy 만 즉시 parse → deeper 발견
        .format = .iife,
    });
    defer bnd.deinit();
    const result = try bnd.bundle(std.testing.io);
    ...
    var has_heavy = false;
    for (seeds) |s| {
        if (std.mem.eql(u8, s, deeper_abs)) has_deeper = true;
        if (std.mem.eql(u8, s, heavy_abs)) has_heavy = true;
    }
    try std.testing.expect(has_deeper); // 중첩 seed 노출 → 재귀 lazy 는 dev 측 누적만으로 성립
    try std.testing.expect(!has_heavy); // heavy 는 force-parse 됨 → seed 아님
}
