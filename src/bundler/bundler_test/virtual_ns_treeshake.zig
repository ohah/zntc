...
test "virtual ns (#1603): import { M } from re_export_namespace — 미사용 member prune" {
    ...
    // 사용된 foo는 남아야 함
    try testing.expect(std.mem.indexOf(u8, code, "foo = 1") != null);
    // 미사용 bar/baz는 tree-shake되어야 함
    try testing.expect(std.mem.indexOf(u8, code, "bar = 2") == null);
    try testing.expect(std.mem.indexOf(u8, code, "baz = 3") == null);
}
...
test "virtual ns (#1603): 여러 member 접근 시 모두 보존" {
    ...
    try testing.expect(std.mem.indexOf(u8, code, "alpha = 1") != null);
    try testing.expect(std.mem.indexOf(u8, code, "gamma = 3") != null);
    try testing.expect(std.mem.indexOf(u8, code, "beta = 2") == null);
    try testing.expect(std.mem.indexOf(u8, code, "delta = 4") == null);
}
...
test "virtual ns (#1603): opaque 사용 시 fallback — 전체 유지" {
    ...
    try testing.expect(std.mem.indexOf(u8, code, "x = 10") != null);
    try testing.expect(std.mem.indexOf(u8, code, "y = 20") != null);
}
...
test "virtual ns (#1603): 여러 소비자가 모두 precise하면 union만 유지" {
    ...
    try testing.expect(std.mem.indexOf(u8, code, "a = 1") != null);
    try testing.expect(std.mem.indexOf(u8, code, "c = 3") != null);
    try testing.expect(std.mem.indexOf(u8, code, "b = 2") == null);
}
