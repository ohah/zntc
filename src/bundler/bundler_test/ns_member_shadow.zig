...
// optional chain(`ns?.x`)도 member-rewrite 대상. module namespace 객체는 절대 nullish 가
// 아니라 ns 위치의 `?.` 는 no-op 이므로 치환 시 안전히 제거된다. 이전엔 rewrite 블록이
// `optional_chain==0` 게이트라 `ns?.x` 전부(존재 멤버 포함) 선언 안 된 ns 로 fall-through
// → ReferenceError 였다.
test "ns optional chain: ns?.member 도 치환 (known→symbol, missing→void 0), dangling 없음" {
    var tmp = testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "mod.js", "export const marker = 1;\nexport function fn() { return 'F'; }");
    try writeFile(tmp.dir, "entry.js",
        \\import * as ns from './mod.js';
        \\globalThis.k = ns?.marker;
        \\globalThis.f = ns?.fn();
        \\globalThis.n = ns?.nope;
    );
    ...
    // 존재 멤버는 symbol 로 치환 → dangling `?.marker` / `?.fn` 이 남으면 안 됨(ReferenceError).
    try testing.expect(std.mem.indexOf(u8, code, "?.marker") == null);
    try testing.expect(std.mem.indexOf(u8, code, "?.fn") == null);
    try testing.expect(std.mem.indexOf(u8, code, "marker = 1") != null);
    // 미존재 멤버는 (void 0).
    try testing.expect(std.mem.indexOf(u8, code, "(void 0)") != null);
}

// optional chain 중간 hop 의 `?.` 는 보존돼야 한다 — `ns?.nope?.deep` 은 첫 hop 만 치환되고
// (`(void 0)`) 뒤 `?.deep` 단락이 유지돼 `(void 0)?.deep` → undefined.
test "ns optional chain: 미존재 멤버 뒤 ?. 단락 보존 (ns?.nope?.deep → (void 0)?.deep)" {
    var tmp = testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "mod.js", "export const marker = 1;");
    try writeFile(tmp.dir, "entry.js",
        \\import * as ns from './mod.js';
        \\globalThis.d = ns?.nope?.deep;
    );
    ...
    // 첫 hop 은 (void 0), 뒤 `?.deep` 는 보존 — `(void 0)?.deep` 형태.
    try testing.expect(std.mem.indexOf(u8, code, "(void 0)?.deep") != null);
    // ns 가 dangling 으로 남으면 안 됨.
    try testing.expect(std.mem.indexOf(u8, code, "ns?.") == null);
}

// optional 미존재 멤버 call (`ns?.nope?.()`)도 `(void 0)` paren 으로 — `void 0?.()` 오파싱 방지.
test "ns optional chain: 미존재 멤버 optional call 은 (void 0)?.() paren 유지" {
    var tmp = testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "mod.js", "export function fn() { return 'OK'; }");
    try writeFile(tmp.dir, "entry.js",
        \\import * as ns from './mod.js';
        \\globalThis.r = ns?.fn?.();
        \\globalThis.m = ns?.nope?.();
    );
    ...
    // 존재 fn 은 symbol → `fn?.()`. 미존재는 `(void 0)?.()`(단락 → undefined).
    try testing.expect(std.mem.indexOf(u8, code, "(void 0)?.()") != null);
    try testing.expect(std.mem.indexOf(u8, code, "ns?.") == null);
}

// over-correction 가드(optional 경로): CJS namespace 는 inner_map 이 없어(copyProps+rename)
// 미존재처럼 보이는 멤버도 `(void 0)` 으로 바꾸면 안 된다 — 런타임 동적 멤버를 가린다.
test "ns optional chain: CJS namespace 의 optional 멤버는 void 0 으로 강제 안 함" {
    var tmp = testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "lib.cjs", "module.exports.foo = 7;");
    try writeFile(tmp.dir, "entry.js",
    ...
    // CJS namespace 는 실제 객체(__toESM/copyProps)라 동적 멤버 접근을 그대로 둔다.
    // `(void 0)` 로 바뀌면 ns.bar 같은 런타임 멤버가 사라진다 — 그러면 안 됨.
    try testing.expect(std.mem.indexOf(u8, code, "(void 0)") == null);
}

// over-correction 가드(optional 경로): 값으로 쓰여 materialize 된 ns 는 미존재 optional
// 멤버도 실제 객체 access 로 두고 `(void 0)` 으로 강제하지 않는다.
test "ns optional chain: materialize 된 ns 의 미존재 optional 멤버는 void 0 강제 안 함" {
    var tmp = testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "mod.js", "export const a = 1;\nexport const b = 2;");
    try writeFile(tmp.dir, "entry.js",
    ...
    // ns 가 인라인 객체로 materialize → 미존재 멤버는 실제 객체 access(undefined), void 0 아님.
    try testing.expect(std.mem.indexOf(u8, code, "get a()") != null);
    try testing.expect(std.mem.indexOf(u8, code, "(void 0)") == null);
}
