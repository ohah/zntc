...
// ============================================================
// #4485 — CSS url() 의 `./` 없는 bare 상대 지정자
//
// CSS 스펙상 `url()` 의 base 는 스타일시트 자신의 URL 이라 `url(logo.png)` 와
// `url(./logo.png)` 는 같은 파일이다. 그런데 bare 는 resolver 가 npm 패키지로
// 오인해 ModuleNotFound → warning + 원문 emit → 런타임 404 였다.
// ============================================================

test "#4485 CSS url(): ./ 없는 bare 도 자산으로 재작성된다" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "entry.ts", "import './style.css';");
    try writeFile(tmp.dir, "style.css",
        \\.a { background: url(./logo.png); }
        \\.b { background: url(logo.png); }
    );
    const img = [_]u8{0x44} ** 5000; // 인라인 한계 초과 → 파일
    try tmp.dir.writeFile(std.testing.io, .{ .sub_path = "logo.png", .data = &img });
    ...
    const css = findCssOutput(result) orelse return error.ExpectedCssOutput;
    // 원문 bare 참조는 사라지고 해시 자산을 가리켜야 한다 (404 방지).
    try std.testing.expect(std.mem.indexOf(u8, css, "url(logo.png)") == null);
    try std.testing.expect(std.mem.indexOf(u8, css, "url(./logo.png)") == null);
    try std.testing.expect(std.mem.indexOf(u8, css, "logo-") != null);

    // `./logo.png` 와 `logo.png` 는 같은 파일 → 자산은 하나만 방출된다.
    const outs = result.asset_outputs orelse return error.ExpectedAssetOutput;
    var png_count: usize = 0;
    for (outs) |o| {
        if (std.mem.endsWith(u8, o.path, ".png")) png_count += 1;
...
test "#4485 CSS url(): scheme / data / root-absolute 는 여전히 무변경" {
    // bare 정규화 확장 뒤에도 이것들은 손대지 않는다. (2중 방어 — css_scanner 의
    // classifyCssUrl 이 애초에 record 를 안 만들고, resolve 레이어의
    // isBareUrlRelativeSpecifier 도 제외한다. 이 테스트는 사용자 눈에 보이는 최종
    // CSS 를 박제한다.)
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "entry.ts", "import './style.css';");
    try writeFile(tmp.dir, "style.css",
        \\.d { background: url(https://cdn.example.com/z.png); }
        \\.e { background: url(data:image/png;base64,iVBORw0KGgo=); }
        \\.f { background: url(/root.png); }
        \\.g { filter: url(#blur); }
        \\.h { background: url(//cdn.example.com/p.png); }
    );
    ...
    const css = findCssOutput(result) orelse return error.ExpectedCssOutput;
    try std.testing.expect(std.mem.indexOf(u8, css, "url(https://cdn.example.com/z.png)") != null);
    try std.testing.expect(std.mem.indexOf(u8, css, "url(data:image/png;base64,iVBORw0KGgo=)") != null);
    try std.testing.expect(std.mem.indexOf(u8, css, "url(/root.png)") != null);
    try std.testing.expect(std.mem.indexOf(u8, css, "url(#blur)") != null);
    try std.testing.expect(std.mem.indexOf(u8, css, "url(//cdn.example.com/p.png)") != null);
}

test "#4485 CSS url(): 없는 bare 자산은 여전히 경고 + 원문 유지 (폴백이 에러를 삼키지 않는다)" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "entry.ts", "import './style.css';");
    try writeFile(tmp.dir, "style.css", ".a { background: url(gone.png); }"); // 파일 없음
    ...
    try std.testing.expect(!result.hasErrors()); // 하드 에러 아님
    // 폴백이 ModuleNotFound 를 null(=external) 로 삼켜버리면 CSS 는 그대로지만 경고가
    // 조용히 사라진다 → 진단 존재까지 박제해야 그 회귀를 잡는다.
    var warned = false;
    for (result.getDiagnostics()) |d| {
        if (std.mem.indexOf(u8, d.message, "Cannot resolve CSS url()") != null) warned = true;
    }
...
test "#4485 CSS url(): --platform=node 에서 builtin 이름과 겹치는 자산 디렉토리도 재작성" {
    // 회귀: `isExternalForKind` 의 Node builtin 자동 external 규칙이 css_url 에도 걸려,
    // `url(path/logo.png)` (자산 디렉토리 이름이 하필 builtin `path` 와 겹침) 가 external 로
    // 빠져 원문 방출됐다 — 경고조차 없이 404. isNodeBuiltin 은 sub-path 도 builtin 으로 친다.
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "entry.ts", "import './style.css';");
    try writeFile(tmp.dir, "style.css", ".a { background: url(path/logo.png); }");
    const img = [_]u8{0x77} ** 5000;
    try writeFile(tmp.dir, "path/.keep", "");
    try tmp.dir.writeFile(std.testing.io, .{ .sub_path = "path/logo.png", .data = &img });
    ...
    try std.testing.expect(!result.hasErrors());
    const css = findCssOutput(result) orelse return error.ExpectedCssOutput;
    try std.testing.expect(std.mem.indexOf(u8, css, "url(path/logo.png)") == null);
    try std.testing.expect(std.mem.indexOf(u8, css, "logo-") != null);
}

test "#4485 CSS url(): builtin 이름과 겹치는 못 찾은 자산은 stub 이 아니라 경고" {
    // 회귀: resolve 실패 시 `browser + isNodeBuiltin(specifier)` 분기가 css_url 까지
    // 빈 CJS stub 으로 삼켜 "Cannot resolve CSS url()" 경고를 없앴다. bare url() 이
    // 형제 파일로 해석되는 지금, 그 경고는 오탈자를 잡아주는 유일한 안전망이다.
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "entry.ts", "import './style.css';");
    try writeFile(tmp.dir, "style.css", ".a { background: url(path/gone.png); }"); // 파일 없음
    ...
    try std.testing.expect(warned);
    const css = findCssOutput(result) orelse return error.ExpectedCssOutput;
    try std.testing.expect(std.mem.indexOf(u8, css, "url(path/gone.png)") != null);
}

test "#4485 CSS url(): bare + ?query / #fragment suffix 도 재작성 + suffix 보존" {
    // css_scanner 가 `?query`/`#fragment` 를 css_url_suffix 로 미리 떼어내므로 resolve 에
    // 도달하는 specifier 는 순수 bare 다 → 정규화 대상. 이 계약이 깨지면(스캐너가 suffix 를
    // specifier 에 남기면) `isBareUrlRelativeSpecifier` 의 `?#` 가드가 폴백을 막아 다시 404.
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "entry.ts", "import './style.css';");
    try writeFile(tmp.dir, "style.css",
        \\@font-face { src: url(f.eot?#iefix) format("embedded-opentype"); }
        \\.i { background: url(sprite.svg#icon); }
    );
    const font = [_]u8{0x66} ** 5000;
    try tmp.dir.writeFile(std.testing.io, .{ .sub_path = "f.eot", .data = &font });
    try writeFile(tmp.dir, "sprite.svg", "<svg><g id=\"icon\"/></svg>");
    ...
    const css = findCssOutput(result) orelse return error.ExpectedCssOutput;
    try std.testing.expect(std.mem.indexOf(u8, css, "url(f.eot?#iefix)") == null);
    try std.testing.expect(std.mem.indexOf(u8, css, ".eot?#iefix\"") != null); // suffix 보존
    try std.testing.expect(std.mem.indexOf(u8, css, ".svg#icon\"") != null); // fragment 보존
}
...
// ============================================================
//
// CSS 스펙상 `url()` 의 상대 참조는 **스타일시트 자신의 URL** 이 base 다 →
// `"logo.png"` 와 `"./logo.png"` 는 같은 파일. 그런데 resolver 는 `./` 없는 지정자를
...
test "#4485 CSS url(): `./` 없는 bare 상대 지정자도 자산으로 재작성된다" {
    ...
    try writeFile(tmp.dir, "entry.ts", "import './style.css';");
    try writeFile(tmp.dir, "style.css",
        \\.dot { background: url(./dot.woff2); }
        \\.bare { background: url(bare.woff2); }
    );
    const font = [_]u8{0x77} ** 5000; // inline limit 초과 → 별도 파일
    try tmp.dir.writeFile(std.testing.io, .{ .sub_path = "dot.woff2", .data = &font });
    const font2 = [_]u8{0x88} ** 5000;
    try tmp.dir.writeFile(std.testing.io, .{ .sub_path = "bare.woff2", .data = &font2 });
    ...
    const css = findCssOutput(result) orelse return error.ExpectedCssOutput;
    // 원문 참조가 남으면 런타임 404.
    try std.testing.expect(std.mem.indexOf(u8, css, "url(bare.woff2)") == null);
    try std.testing.expect(std.mem.indexOf(u8, css, "url(./dot.woff2)") == null);
    try std.testing.expect(std.mem.indexOf(u8, css, "bare-") != null);
    try std.testing.expect(std.mem.indexOf(u8, css, "dot-") != null);
}

test "#4485 CSS url(): scheme / data / root-absolute 는 건드리지 않는다" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "entry.ts", "import './style.css';");
    try writeFile(tmp.dir, "style.css",
    ...
    try std.testing.expect(!result.hasErrors());
    const css = findCssOutput(result) orelse return error.ExpectedCssOutput;
    // origin 기준 참조는 base 가 스타일시트가 아니다 → 원문 유지가 정답.
    try std.testing.expect(std.mem.indexOf(u8, css, "https://cdn.example/x.png") != null);
    try std.testing.expect(std.mem.indexOf(u8, css, "data:image/png;base64,AA") != null);
    try std.testing.expect(std.mem.indexOf(u8, css, "url(/abs.png)") != null);
}
