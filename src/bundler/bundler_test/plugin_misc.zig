...
test "RN preset: --rn-version 0.84 는 class/async/optional-chaining 네이티브 유지 (정밀 매트릭스)" {
    // 진실 소스 = reactnative.dev/docs/<ver>/javascript-environment. 문서에 있는 기능은
    // 다운레벨하지 않는다 — blunt fromHermesPreset 과 달리 class/async 가 ES5 로 안 내려감.
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "entry.js",
        \\export class Foo extends Bar {
        \\  x = 1;
        \\  async run() { return await this.go(); }
        \\}
        \\export const g = obj?.a ?? "d";
    );
    const entry = try absPath(&tmp, "entry.js");
    defer std.testing.allocator.free(entry);
    ...
    var b = Bundler.init(std.testing.allocator, .{
        .entry_points = &.{entry},
        .platform = .react_native,
        .rn_version_matrix = compat.fromReactNativeVersion(0, 84),
    });
    defer b.deinit();
    const result = try b.bundle(std.testing.io);
    defer result.deinit(std.testing.allocator);
    ...
    try std.testing.expect(!result.hasErrors());
    // class 네이티브 (ES5 function 으로 변환 안 됨)
    try std.testing.expect(std.mem.indexOf(u8, result.output, "class Foo extends Bar") != null);
    // async 네이티브 (state machine 변환 안 됨)
    try std.testing.expect(std.mem.indexOf(u8, result.output, "async run") != null);
    // optional chaining / nullish 네이티브
    try std.testing.expect(std.mem.indexOf(u8, result.output, "?.") != null);
    try std.testing.expect(std.mem.indexOf(u8, result.output, "??") != null);
}

test "RN preset: --rn-version 0.84 는 문서 미기재 다운레벨 + #1299 안전망 (정밀 매트릭스)" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "entry.js",
        \\export let a = 1;
    ...
    try std.testing.expect(!result.hasErrors());
    // 유저 코드 기준 검증 (런타임 헬퍼 스타일은 configurable_exports 소관, 본 기능과 무관).
    // logical assignment (ES2021) — 문서 미기재 → 다운레벨 (||= 사라짐)
    try std.testing.expect(std.mem.indexOf(u8, result.output, "||=") == null);
    // #1299 안전망: 유저 arrow → function (문서엔 지원이지만 Hermes 런타임 버그로 강제 다운레벨)
    try std.testing.expect(std.mem.indexOf(u8, result.output, "fn = function") != null);
    // #1299 안전망: 유저 for-let → for-var
    try std.testing.expect(std.mem.indexOf(u8, result.output, "for (let") == null);
}
