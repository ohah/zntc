...
test "Bundler: try-block 안의 unresolved require 는 warning 만 + stub 으로 대체" {
    // `try { require("debug") } catch {}` 같은 Metro 식 optional dependency 패턴.
    // axios → follow-redirects/debug.js 가 silent-catch 로 unresolved 를 흡수하므로
    // ZNTC 가 hard error 로 처리하면 axios 등 흔한 패키지 빌드 자체가 실패한다.
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "entry.js",
        \\let dbg;
        \\try { dbg = require("missing-pkg-xyz"); } catch (e) {}
        \\const x = require("./real.js");
        \\module.exports = { dbg: dbg, x: x };
    );
    try writeFile(tmp.dir, "real.js", "module.exports = 42;");
    ...
    try std.testing.expect(!result.hasErrors());
    // unresolved warning 은 들어와야 함.
    const diags = result.getDiagnostics();
    var has_optional_warning = false;
    for (diags) |d| {
        if (d.code == .unresolved_import and d.severity == .warning) {
            has_optional_warning = true;
            break;
        }
    }
    try std.testing.expect(has_optional_warning);
    // 정상 require ("./real.js") 는 그대로 해석.
...
test "Bundler: try-block 바깥의 unresolved require 는 여전히 hard error (회귀 가드)" {
    ...
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "entry.js", "const x = require('missing-pkg-xyz');");
...
test "Bundler: ESM external default import 가 모듈마다 mangled local 일 때 alias var emit" {
    // axios 의 ZlibHeaderTransformStream.js 패턴 (#2027): 두 모듈이 같은 native specifier
    // 를 default import 하지만 같은 chunk 안에서 collision 으로 두 번째가 mangled
    // (e.g. `stream$1`) 되어 declaration 누락되던 dangling reference 회귀 가드.
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    // 두 모듈 모두 default 를 동일 이름 `stream` 으로 import — 같은 chunk 로 합쳐지면
    // linker mangler 가 후자를 `stream$1` 로 rename 하도록 강제.
    try writeFile(tmp.dir, "a.js",
        \\import stream from 'stream';
        \\const localStream = stream;
        \\export const A = localStream.Readable;
    );
    try writeFile(tmp.dir, "b.js",
        \\import stream from 'stream';
        \\const localStream = stream;
        \\export const B = localStream.Transform;
    );
    try writeFile(tmp.dir, "entry.js",
        \\import { A } from './a';
        \\import { B } from './b';
    ...
    try std.testing.expect(!result.hasErrors());
    // ESM import 한 줄 (default 는 슬롯 한 개)
    ...
    // 두 번째 default 는 mangled 되어 alias var declaration 으로 보강돼야 함.
    // 변수 이름 자체 (`stream$1`) 는 linker 의 mangling 휴리스틱에 의존하므로 패턴
    // 검사 (`var <id>=stream` 또는 `var <id> = stream`) 로 mangled alias 존재 보장.
    const has_alias_min = std.mem.indexOf(u8, result.output, "=stream;") != null;
    const has_alias_pretty = std.mem.indexOf(u8, result.output, " = stream;") != null;
    try std.testing.expect(has_alias_min or has_alias_pretty);
}
...
test "Bundler: require.context IIFE has no raw require call (RN runtime invariant)" {
    ...
    while (std.mem.indexOfPos(u8, result.output, pos, start_marker)) |start| {
        ...
        // raw require 호출 패턴 (앞에 `.` 같은 멤버 access 없는 경우만 검사 — `__zntc_modules[...]`
