...
test "computeRenames: rename된 심볼의 canonical_name이 semantic.Symbol에 미러됨" {
    ...
    // RFC #3940 Sub-PR-L.3 — 병행 작성 parity: computeRenames (putCanonicalName 경로) 가
    // canonical_name 과 rename_table 에 동일하게 기록했는지. rename_table 엔트리 수도
    // canonical_name 보유 심볼 수와 일치해야 함.
    try std.testing.expect(r.linker.renameTableParityHolds());
    try std.testing.expect(r.linker.rename_table.count() >= renamed_count);
}

test "RenameTable 병행: computeMangling 경로도 parity 유지 (RFC #3940 L.3)" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    // computeMangling (assignSymbolCanonical 직접 경로) 가 실제로 rename 을 만들고
    // (vacuous parity 방지), 두 곳이 일치하는지.
    try std.testing.expect(r.linker.rename_table.count() > 0);
    try std.testing.expect(r.linker.renameTableParityHolds());
}

test "RenameTable 병행: clearCanonicalNames 가 rename_table 도 비움 (RFC #3940 L.3)" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "a.ts", "export { x as a } from './m1'; export { x as b } from './m2';");
    try writeFile(tmp.dir, "m1.ts", "export const x = 1;");
