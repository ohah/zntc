...
test "loadContract: 부재 → MfContractManifestNotFound, 디렉터리(IsDir) → MfContractMalformed" {
    ...
    try std.testing.expectError(
        ...
        loadContract(std.testing.io, a, null, "/zntc-nonexistent-xyzzy/mf-manifest.json"),
    );
    // read-fail(권한/IO 류) — 디렉터리를 manifest 경로로 → IsDir → 결함
    var td = std.testing.tmpDir(.{});
    defer td.cleanup();
    try td.dir.createDirPath(std.testing.io, "d.json");
    const base = try td.dir.realPathFileAlloc(std.testing.io, ".", a);
    defer a.free(base);
    const dirpath = try std.fs.path.join(a, &.{ base, "d.json" });
    defer a.free(dirpath);
    try std.testing.expectError(error.MfContractMalformed, loadContract(std.testing.io, a, null, dirpath));
}
