...
// ── PR-2 runtime_polyfill_roots + run_before_main 위상 보존 ─────────────────

const rp_t = @import("runtime_polyfills.zig");

fn preservedBuildPoly(
    graph: *ModuleGraph_t,
    store: *module_store.PersistentModuleStore,
    rc: *@import("resolve_cache.zig").ResolveCache,
    entry: []const u8,
...
fn freshFullPoly(entry: []const u8, plan: ?rp_t.Plan, rbm: []const []const u8) !BundleResult {
    var fresh = Bundler.init(std.testing.allocator, .{
        .entry_points = &.{entry},
        .dev_mode = true,
        .runtime_polyfills = plan,
...
test "PR-2: usage polyfill body-only(feature 불변) → 보존 + prelude == fresh full" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "pf_pas.js", "globalThis.__pf_pas = 1;");
    try writeFile(tmp.dir, "index.ts",
    ...
    const entry = try absPath(&tmp, "index.ts");
    defer std.testing.allocator.free(entry);
    const stub = try absPath(&tmp, "pf_pas.js");
    defer std.testing.allocator.free(stub);

    const candidates = [_]rp_t.Candidate{
        .{ .feature = "promise_all_settled", .module = "core-js/es.promise.all-settled", .path = stub },
    };
    const plan = rp_t.Plan{ .mode = .usage, .candidates = &candidates };

    var rc = Bundler.initResolveCacheFromOptions(std.testing.allocator, .{ .entry_points = &.{entry}, .dev_mode = true, .runtime_polyfills = plan });
    defer rc.deinit();
    var store = module_store.PersistentModuleStore.init(std.testing.allocator);
    defer store.deinit();
    var graph = ModuleGraph_t.init(std.testing.allocator, &rc);
    ...
    {
        var r1 = try preservedBuildPoly(&graph, &store, &rc, entry, plan, &.{}, true, null);
        defer r1.deinit(std.testing.allocator);
        try std.testing.expect(!r1.hasErrors());
        // Promise.allSettled → candidate 매칭 → stub 이 roots 에 선택됐는지(전제).
        try std.testing.expect(graph.runtime_polyfill_roots.items.len == 1);
    }
    const hits_before = graph.topology_preserved_hits;
    const fb_before = graph.topology_fallback_count;
    ...
    try writeFile(tmp.dir, "index.ts",
        \\const r = Promise.allSettled([]);
        \\console.log(r, 'edited');
    );
    var touched: std.StringHashMapUnmanaged(void) = .empty;
    defer touched.deinit(std.testing.allocator);
    try touched.put(std.testing.allocator, entry, {});

    var preserved = try preservedBuildPoly(&graph, &store, &rc, entry, plan, &.{}, false, &touched);
    defer preserved.deinit(std.testing.allocator);
    try std.testing.expect(!preserved.hasErrors());

    var fresh = try freshFullPoly(entry, plan, &.{});
    defer fresh.deinit(std.testing.allocator);
    try std.testing.expect(!fresh.hasErrors());

    // feature 불변 → 보존(hit++/fb 불변) + polyfill prelude byte-identical.
    try std.testing.expectEqual(hits_before + 1, graph.topology_preserved_hits);
    try std.testing.expectEqual(fb_before, graph.topology_fallback_count);
    try std.testing.expectEqualStrings(fresh.output, preserved.output);
}

test "PR-2: usage polyfill feature 추가(새 core-js API) → fallback + byte-identical" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "pf_pas.js", "globalThis.__pf_pas = 1;");
    try writeFile(tmp.dir, "pf_oho.js", "globalThis.__pf_oho = 1;");
    try writeFile(tmp.dir, "index.ts",
        \\const r = Promise.allSettled([]);
        \\console.log(r);
    );
    const entry = try absPath(&tmp, "index.ts");
    defer std.testing.allocator.free(entry);
    const stub_pas = try absPath(&tmp, "pf_pas.js");
    defer std.testing.allocator.free(stub_pas);
    const stub_oho = try absPath(&tmp, "pf_oho.js");
    defer std.testing.allocator.free(stub_oho);

    const candidates = [_]rp_t.Candidate{
        .{ .feature = "promise_all_settled", .module = "core-js/es.promise.all-settled", .path = stub_pas },
        .{ .feature = "object_has_own", .module = "core-js/es.object.has-own", .path = stub_oho },
    };
    const plan = rp_t.Plan{ .mode = .usage, .candidates = &candidates };
...

    {
        ...
        defer r1.deinit(std.testing.allocator);
        try std.testing.expect(!r1.hasErrors());
        try std.testing.expect(graph.runtime_polyfill_roots.items.len == 1); // pas 만
    }
    const fb_before = graph.topology_fallback_count;

    // Object.hasOwn 추가 → 변경 모듈 feature set 변화 → feature-diff fallback.
    std.testing.io.sleep(std.Io.Duration.fromMilliseconds(50), .awake) catch {};
    try writeFile(tmp.dir, "index.ts",
        \\const r = Promise.allSettled([]);
        \\const o = Object.hasOwn({}, "a");
        \\console.log(r, o);
    );
    var touched: std.StringHashMapUnmanaged(void) = .empty;
    defer touched.deinit(std.testing.allocator);
...
test "PR-2: entry mode polyfill → feature 변해도 항상 보존 + byte-identical" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "pf.js", "globalThis.__pf = 1;");
    try writeFile(tmp.dir, "index.ts",
        \\console.log(1);
    );
    const entry = try absPath(&tmp, "index.ts");
    defer std.testing.allocator.free(entry);
    const stub = try absPath(&tmp, "pf.js");
    defer std.testing.allocator.free(stub);

    const entry_mods = [_]rp_t.ResolvedModule{.{ .module = "core-js/stub", .path = stub }};
    const plan = rp_t.Plan{ .mode = .entry, .entry_modules = &entry_mods };
    ...
    // entry mode 는 고정 plan → feature 가 변해도(여기선 추가) feature-diff skip → 보존.
    std.testing.io.sleep(std.Io.Duration.fromMilliseconds(50), .awake) catch {};
    try writeFile(tmp.dir, "index.ts",
        \\const r = Promise.allSettled([]);
        \\console.log(r);
...
test "PR-2: run_before_main 고정 → body-only edit 보존 + prelude(rbm) == fresh full" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "rbm.js", "globalThis.__rbm = 1;");
    try writeFile(tmp.dir, "index.ts",
        \\console.log(1);
    );
    const entry = try absPath(&tmp, "index.ts");
    defer std.testing.allocator.free(entry);
    const rbm = try absPath(&tmp, "rbm.js");
    defer std.testing.allocator.free(rbm);
    const rbm_list = [_][]const u8{rbm};

    var rc = Bundler.initResolveCacheFromOptions(std.testing.allocator, .{ .entry_points = &.{entry}, .dev_mode = true, .run_before_main = &rbm_list });
    defer rc.deinit();
    var store = module_store.PersistentModuleStore.init(std.testing.allocator);
    defer store.deinit();
    var graph = ModuleGraph_t.init(std.testing.allocator, &rc);
    ...
    {
        var r1 = try preservedBuildPoly(&graph, &store, &rc, entry, null, &rbm_list, true, null);
        defer r1.deinit(std.testing.allocator);
        try std.testing.expect(!r1.hasErrors());
    }
    const hits_before = graph.topology_preserved_hits;
    ...
    var preserved = try preservedBuildPoly(&graph, &store, &rc, entry, null, &rbm_list, false, &touched);
    defer preserved.deinit(std.testing.allocator);
    try std.testing.expect(!preserved.hasErrors());

    var fresh = try freshFullPoly(entry, null, &rbm_list);
    defer fresh.deinit(std.testing.allocator);
    try std.testing.expect(!fresh.hasErrors());

    // run_before_main(고정)이 보존돼 prelude 순서/내용이 fresh 와 동일.
    try std.testing.expectEqual(hits_before + 1, graph.topology_preserved_hits);
    try std.testing.expectEqual(fb_before, graph.topology_fallback_count);
    try std.testing.expectEqualStrings(fresh.output, preserved.output);
}

test "PR-2: polyfill 스텁 자체 변경 → 보존 reparse(is_context_dep 복원) + == fresh full" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeFile(tmp.dir, "pf_pas.js", "globalThis.__pf_pas = 1;");
    try writeFile(tmp.dir, "index.ts",
    ...
    // polyfill 스텁(is_context_dep) 자체 변경 → reparse 가 is_context_dep 를 복원해 fallback 없이 보존.
    std.testing.io.sleep(std.Io.Duration.fromMilliseconds(50), .awake) catch {};
    try writeFile(tmp.dir, "pf_pas.js", "globalThis.__pf_pas = 2;");
    var touched: std.StringHashMapUnmanaged(void) = .empty;
    defer touched.deinit(std.testing.allocator);
    try touched.put(std.testing.allocator, stub, {});
    ...
    // 스텁 변경도 fallback 없이 보존(hit++/fb 불변) + 새 스텁 코드가 prelude 에 byte-identical.
    try std.testing.expectEqual(hits_before + 1, graph.topology_preserved_hits);
    try std.testing.expectEqual(fb_before, graph.topology_fallback_count);
    try std.testing.expectEqualStrings(fresh.output, preserved.output);
}
