...
const emitter_tree_shake = @import("emitter/tree_shake.zig");
...
pub const OutputFile = struct {
    ...
    /// dev+split (RFC_LAZY_DEV_MODULE_HMR PR-1): 이 청크에 포함된 모듈들의
    /// per-module HMR code. `dev_mode and collect_module_codes` 일 때만 채워진다.
    /// caller(bundler.zig)가 전체 청크에서 평탄화해 BundleResult.module_dev_codes
    /// 로 노출 → onRebuild 의 updates 계산에 사용. null 이면 미수집(production/비-dev).
    /// allocator 소유 — deinit 에서 freeAll.
    module_dev_codes: ?[]const types.ModuleDevCode = null,
    ...
    pub fn deinit(self: OutputFile, allocator: std.mem.Allocator) void {
        ...
        if (self.module_dev_codes) |codes| types.ModuleDevCode.freeAll(codes, allocator);
    }
...
pub fn emitWithTreeShaking(
    ...
    if (options.line_limit > 0) {
        var wrap_breaks: std.ArrayList(line_wrap.WrapBreak) = .empty;
        defer wrap_breaks.deinit(allocator);
        try line_wrap.wrapLineLimit(&output, allocator, options.line_limit, &wrap_breaks);
        if (bundle_sm) |*sm| {
            if (wrap_breaks.items.len > 0) {
                line_wrap.adjustMappingsForLineWraps(sm, wrap_breaks.items);
            }
        }
    }
...
/// dev HMR per-module code wrap: IIFE + 런타임 헬퍼 로컬 alias.
/// `__zntc_apply_update` 가 eval 하는 단위. 단일 번들(emitWithTreeShaking)과
/// split(chunks.emitChunks)이 *동일* 형식을 쓰도록 공유한다 (HMR client 호환).
/// `with_source_url=true` 면 끝에 `//# sourceURL=<mod_id>` 를 덧붙여 DevTools 가
/// 익명 eval(VM:1) 대신 모듈 경로로 표시하게 한다. IIFE 끝 뒤라 preamble 오프셋 무관.
pub fn wrapDevModuleCode(
    allocator: std.mem.Allocator,
    code: []const u8,
    mod_id: []const u8,
    with_source_url: bool,
) ![]const u8 {
...
pub fn emitModule(
    ...
    if (linker) |l| {
        ...
        if (!options.dev_mode) {
            ...
            if ((module.wrap_kind == .esm or options.format == .esm) and options.minify_syntax) {
                if (module.semantic) |sem| {
                    dead_store.markDeadOverwrittenFunctionBodiesOnly(
                        arena_alloc,
                        transformer.ast,
                        md.symbol_ids,
                        sem.references,
                                ...
                                if (new_root.tag == .program and new_root.data.list.len > 0) {
                                    ...
                                    if (new_list.start + new_list.len <= transformer.ast.extra_data.items.len) {
                                        ...
                                        if (!s.isExportUsed(mod_idx, "*")) {
                                            emitter_tree_shake.markUnusedCjsObjectProperties(
                                                arena_alloc,
                                                module,
                                                transformer.ast,
                                                ts_infos,
                                        ...
                                        if (options.minify_syntax) {
                                            dead_store.markDeadOverwrittenAssignments(
                                                arena_alloc,
                                                transformer.ast,
                                                md.symbol_ids,
                                                sem.references,
    ...
    if (linker) |l| {
        if (!options.dev_mode) {
            ...
            emitter_tree_shake.propagateCrossModulePurity(l, module, transformer.ast, sym_ids, arena_alloc);
        }
    }
    ...
    var final_exports_buf: ?[]const u8 = null;
...
const emitOutputExportsConflictDiag = entry_exports.emitOutputExportsConflictDiag;
const emitEsmEntryExports = entry_exports.emitEsmEntryExports;
const emitWrappedEntryExports = entry_exports.emitWrappedEntryExports;
const emitCjsEntryExports = entry_exports.emitCjsEntryExports;
