...
test "resolveExports: 매칭된 조건 null target 은 blocked — sibling/default 폴백 금지 (#3981)" {
    const a = std.testing.allocator;
    const source =
        \\{"exports":{".":{"browser":null,"import":"./esm.js","default":"./index.js"}}}
    ;
    const parsed = try std.json.parseFromSlice(std.json.Value, a, source, .{});
    defer parsed.deinit();
    const exports = parsed.value.object.get("exports").?;

    // browser 조건 매칭 → null target → blocked (import/default 로 fall-through 안 함)
    const blocked = resolveExports(a, exports, ".", &.{ "browser", "import", "default" });
    try std.testing.expect(blocked != null);
    try std.testing.expect(blocked.?.blocked);

    // node 플랫폼(browser 조건 없음) → browser key 미매칭 → import 정상 해석
    const ok = resolveExports(a, exports, ".", &.{ "import", "default" });
    try std.testing.expect(ok != null);
    try std.testing.expect(!ok.?.blocked);
    try std.testing.expectEqualStrings("./esm.js", ok.?.path);
}
