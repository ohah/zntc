...
`zts build`는 HTML의 `<script type="module" src>`를 엔트리로 사용하고,
...
기본 prefix는 `VITE_`, `ZTS_`이다. `import.meta.env.MODE/DEV/PROD/SSR/BASE_URL`,
...
```bash
zts --bundle src/index.ts --drop-labels=DEV,TEST
zts --bundle src/index.ts --pure:React.createElement --pure:PropTypes.*
```
...
```typescript
import { init, transpile, buildSync, build, buildAppSync, prepareAppDevSync, watch, vitePlugin } from "@zts/core";
init();
...
# JSON 출력 (스크립트 용)
zts bundle entry.ts --profile=all --profile-format=json

# phase 격리 실행 (debug: 지정 phase 이후 skip)
zts input.ts --stop-after=parse --profile=parse

# 반복 측정 + 통계 (oxc/swc 에도 없는 CLI 벤치마크)
zts bench --phase=parse ./src/App.tsx --iterations=100
zts bench --phase=parse ./src/App.tsx --compare=./baseline.json
```
