...
| 레이어 | 언어 | 책임 | 안정성 |
|---|---|---|---|
| **Zig/C 코어** (`zntc-rn-loader-core`) | Zig | 매니페스트 파싱, 캐시 인덱스/축출, 해시 계산, Ed25519 서명 검증, 오프라인 결정, 상태머신(resolve→캐시확인→fetch/serve→verify→evaluate) | iOS/Android 동일 바이너리 재사용 |
...
- **검증(실행 *전*, 네이티브에서)**: 빌드타임에 청크/.hbc content hash → Ed25519 서명(개인키는 CI 시크릿; Zig 0.15.2 std.crypto 에 RSA sign 부재 → RS256 대신 Ed25519 채택, §8.2.4 동일). 앱 바이너리에 공개키 임베드. 런타임에 Ed25519 서명 검증 + 본문 해시 대조, 통과 시에만 엔진 전달, 실패 시 거부+fail-soft
...
| 서명/무결성 인프라 | ❌ content hash(Wyhash)만. sha256/SRI/서명 전무 | **신규**: SHA-256 SRI + Ed25519 서명/검증 |
...
| **P2** | 웹 MF MVP: shared scope + 서명 매니페스트 에미터 + `remoteEntry`. SHA-256/Ed25519 서명 인프라. metafile 확장. `@module-federation/enhanced` interop 검증 | 없음 |
...
| **P2-3** | Ed25519 서명 에미터 + verify (manifest 무결성 한정, 표준 부재 → zntc 고유·D3 인접; Zig 0.15.2 RSA sign 부재로 RS256→Ed25519; 런타임 강제 verify 는 P4/§8.2.4) | §4.1·§5.4·§9 | P2-2 |
...
#### 8.2.1 증명 대상 = 사용자 계약 (designed shape)

스파이크는 *추상 메커니즘*이 아니라 **이 사용자 계약이 실제 동작함**을 목표로 한다(웹과 동일 소비 문법 + RN 불가피 추가 1점):

- 빌드: `zntc.config` `mf` 블록(웹과 동일) + `--platform=react-native`. remote=컨테이너 산출.
- 소비 코드: **표준 MF 그대로** — `React.lazy(()=>import('remote/X'))` / `await import('remote/X')` / 표준 `loadRemote`. zntc 전용 문법 0(D1).
- RN 유일 추가: 네이티브 로더를 표준 `@module-federation/runtime` 에 **플러그인으로 1회 동기 등록**(첫 lazy 전, `index.js` 최상단). `ScriptManager` 류 zntc 전용 런타임 API 클론 금지(D1) — 표준 runtime + 얇은 플러그인.
- 패키지: 일반 `@zntc/react-native`(JS-only, 누구나 설치) 서브패스 ❌ — 네이티브 autolink 는 **패키지 입자**라 비-MF 사용자에 새어듦. → MF **opt-in 별 패키지**(JS register + 네이티브 동거)의 서브패스. 소스 `.ts` 작성 → 발행 `exports` 는 컴파일 `.js`+`.d.ts`(기존 `@zntc/react-native` 관례).

#### 8.2.2 스파이크0 go/no-go 검증 항목

전부 살아있는 RN 런타임에서만 진위가 드러남(읽어서 확인 불가가 스파이크 사유). go/no-go 핵심은 B1·B2·B3:

| # | 항목 | 실패 시 |
|---|---|---|
| B1 | `.hbc` 바이트코드-버전 일치(emit `.hbc` ↔ 호스트 앱 Hermes) | startup 가치 붕괴 → JS-only 폴백 재검토 |
| B2 | **완료값=컨테이너 ABI(②)** — `evaluateJavaScript` 가 hbc 의 top-level 완료 `jsi::Value` 를 JS 소스와 동일 반환? | ② 무효 → globalThis 폴백(= Re.Pack 동급, 차별화 없음 → RN 진입 재고) |
| B3 | shared singleton — 주입 remote 가 host 와 동일 React 인스턴스(렌더·hooks) | MF 자체 불성립 |
| B4 | **RuntimeExecutor-only(①)** — New Arch 정식 `RuntimeExecutor` 로 `jsi::Runtime&` 획득(private `bridge.runtime` 캐스팅·멀티버전 shim 회피) | Re.Pack 식 private 경로로 후퇴(취약 표면 증가) |
| B5 | JS 소스 주입·등록·렌더(세션 중) | 기본 메커니즘 불성립 |
| B6 | Value 마샬링/스레딩 — CallInvoker async 홉에서 Value 캡처→Promise resolve | 간헐 크래시 |
| B7 | 표준 runtime **플러그인 hook 맞물림** + resolver **동기 등록 순서**(첫 `React.lazy` 전) — 누락 시 첫 lazy 실패(RN 공통 함정) | 사용자 계약 8.2.1 미성립 |
| B8 | webpack-origin(globalThis) ↔ zntc(②) remote 가 한 로더에 공존(interop) / 실패·teardown 시 런타임 무오염 | 생태계 호환·안정성 |

#### 8.2.3 ①RuntimeExecutor-only · ②완료값 ABI = 차별화 가설

Re.Pack 실측(`NativeScriptLoader`/JSI)으로 메커니즘은 기지(旣知). zntc 차별점은 **emit·로더 양 끝을 소유**(D1/D4)해서만 가능한 두 개선:

- **① RuntimeExecutor-only**: 그린필드라 구브릿지 호환 의무 없음 → New Arch `RuntimeExecutor` 정식 핸드오프만(Re.Pack 의 `versioned/CallInvoker` shim 동물원 소거). 취약 표면 최소화.
- **② 완료값=컨테이너**: 리모트 tail 이 컨테이너를 *반환* → 로더가 `evaluateJavaScript` 반환 `Value` 캡처. webpack 규약의 `globalThis.<name>` 부수효과(이름충돌·경쟁·잔존·격리붕괴 버그클래스)를 *소거*. webpack-origin remote 는 globalThis 폴백(B8) — 양 ABI 공존.
- 무손실/하방 없음: ②가 hbc 에서 안 되면(B2 실패) globalThis 폴백 = Re.Pack 동급(손실 0, 상방만).

#### 8.2.4 보안 모델 (P4 — §7.2 ②/§7.3 ③ 이연분 구체화)

- **달성 가능 = 무결성·진정성**(서명/해시 → evaluate *전* 검증 → 불일치 실행거부+에러). **불가 = 기밀성**(디바이스가 실행하려면 복호화·키 on-device; 클라이언트 코드는 본질적으로 가독). 약속은 "변조 시 인증실패로 실행거부(tamper-evident)"이지 "사용자가 못 봄"이 아님.
- 빌드타임 D3(이미 구현: SHA-256/Ed25519 manifest 무결성, 호스트 빌드 fail-fast)은 CI 시점. **런타임 디바이스 검증**(fetch→본문 서명검증→통과해야 `evaluateJavaScript`, 불일치 throw)은 D4 네이티브 로더에 박는 P4 신규 — primitive(computeSri/Ed25519) 재사용.
- 결정 항목: 검증 공개키 **앱 바이너리 내장**(네이티브 in-app 이어야 하는 추가 사유) / manifest 다이제스트만 아닌 **remoteEntry·청크 *본문* 서명**(RN 은 본문이 디바이스서 직접 eval) / 불일치 시 hard-fail vs 런타임 가드 폴백(셸 생존) 정책. Re.Pack 도 code signing 보유(`CodeSigningUtils` android/ios) — zntc 는 빌드 D3 + 런타임 본문서명 = 한 겹 더.

#### 8.2.5 배포 아키텍처

- **하이브리드**: RN-결합·취약(JSI evaluate, TurboModule glue, CallInvoker)=**thin 소스 shim**(앱 빌드시 재컴파일 → RN 업글 생존; ①RuntimeExecutor-only 라 작음) + RN-ABI 무관 무거운 코어(fetch/캐시/서명/매니페스트=Zig 강점)=**prebuilt static lib**(arch-만 매트릭스, RN버전 무결합, Zig 툴체인 은닉). RFC D4("Zig/C 코어 + JSI + thin shim")와 정확히 일치. 전부 prebuilt ❌(RN JSI/Hermes ABI 불안정 — Re.Pack 이 prebuilt 안 쓰는 이유).
- **호스트-번들-비소유 원칙(핵심 차별 결정)**: 호스트는 **표준 단일 Metro 번들**(MF 런타임 + loadRemote 호출부뿐) 유지, zntc 네이티브 로더는 **원격 remote 로딩만** 소유. → CodePush/EAS Update 가 호스트 번들을 온전히 소유 → **공존**. Re.Pack 이 CodePush 와 실무 충돌하는 건 Re.Pack 이 호스트까지 쪼개고 ScriptManager 가 전 스크립트 전달을 장악(자원 이중소유)해서 — zntc 는 호스트 비소유로 그 충돌 구조적 회피(공식 "불가" 명문은 없음 = 구현 충돌이지 MF 본질 아님).
- **OTA 분리**: 네이티브 로더=앱 바이너리 내장(릴리스 고정), remote 청크=CDN/HTTP 런타임 로드(OTA). `EAS Update ≠ MF`(전자=내 앱 번들 한 덩어리 OTA, 후자=독립 배포 remote) — 문서에 구분 명시.
- **Expo**: config plugin 제공 → `expo prebuild`/EAS Build 가 autolink 반영. **Expo Go ❌**(커스텀 네이티브 일반 제약, MF 특정 불가 아님), dev build/EAS ✅. Expo *프레임워크*(Expo Router/EAS 깊은 결합)는 별 어댑터 축(로드맵).
- **`.hbc`-Hermes 결합**: remote `.hbc` 는 호스트 앱 Hermes 와 일치 필요(아니면 JS 소스 폴백) — 네이티브 패키징과 별개인 *콘텐츠 배포* 제약. B1 이 이를 증명.

> 위 8.2.1~8.2.5 는 **designed·미구현·스파이크0 게이트**. 실 RN 네이티브 환경(로컬 Xcode/NDK·Hermes·New Arch) 확보가 실질 선결. 스파이크 통과 후 산출 폐기, 동작은 영구 테스트 박제(§8 디리스크 패턴).
...
- 계약 검증 시점: 웹은 빌드 핀 + 런타임 가드로 **완결**(P1~P3). RN 런타임 디바이스 본문 서명검증은 §8.2.4 로 구체화(P4 게이트)
