...
```zig
// src/bundler/module_list.zig 의 커스텀 자료구조 (graph/state.zig 정의).
pub const ModuleList = StableSegmentedList(Module);
...
**핵심 성질**: `StableSegmentedList` 는 `append` 시 새 chunk 만 alloc 하고
기존 chunk 는 건드리지 않으며, **shelf pointer 배열 자체도 고정 크기**라
append 가 절대 realloc 을 일으키지 않는다. 결과로 **한 번 등록된 `*Module`
포인터는 그래프 수명 동안 영구 유효**. 이 성질이 worker race-safety 의 근간이다.

> **주의 — `std.SegmentedList` 를 쓰면 안 된다.** `std.SegmentedList(Module, 0)`
> 은 chunk 내부 pointer 는 stable 하지만 shelf pointer 를 보관하는
> `dynamic_segments` (`ArrayListUnmanaged`) 가 grow 시 realloc 된다. worker
> thread 가 `at(i)` 로 그 shelf 배열을 읽는 중이면 stale pointer → segfault
> (CI Stress 테스트에서 재현된 race). 그래서 커스텀 `StableSegmentedList`
> (shelf 배열을 고정 크기 `[MAX_SHELVES]?[*]T` 로 선언) 로 교체됐다. 배경은
> `src/bundler/module_list.zig` / `src/bundler/graph.zig` 헤더 참조.

비교로 더 이전의 `std.ArrayList(Module)` 은 capacity 초과 시 realloc → memcpy 가
일어나서, worker 가 capture 한 포인터를 dangling 으로 만들었다.

### 금지

- 외부 코드가 `graph.modules` 에 직접 접근 **금지**. §3 의 공개 API 경유.
- 내부 코드 (graph.zig) 라도 slice 전달 (`graph.modules.items`) 금지 — 이미
  `StableSegmentedList` 라서 불가능하지만, 혹시 회귀하지 않도록 명시.
...
- inner pointer capture: `&module.ast`, `&module.parse_arena`,
  ...
  이유: 이들은 storage 자료구조와 무관하게 module 이 다른 chunk 로 이동하면
...
`StableSegmentedList.ConstIterator` 가 internal segment pointer 를 캐싱해 chunk
...
- `#1779` 에픽 PR: `#1780` (accessor 뼈대), `#1781` (call site 교체),
  `#1782` (slice API 제거), `#1783` (segmented storage 도입), `#1784` (ModuleList 상수).
  이후 `std.SegmentedList` 의 shelf pointer 배열 unstable 문제가 발견돼 커스텀
  `StableSegmentedList` 로 교체됐다 (`src/bundler/module_list.zig`).
