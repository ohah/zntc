...
#    changesets 가 못 보는 root / platform 9개 / core 의 optionalDependencies 는
#    sync-release-versions.ts 가 자동으로 맞춘다 (§6). 손으로 올릴 것 없음.
...
- `baseBranch: "main"`
- `fixed`: main 7개 (`@zntc/core` / `web` / `vite-plugin` / `rspack-loader` / `react-native` / `init` / `wasm`) 를 한 그룹으로 묶어 **lockstep version** 강제. 어느 패키지 changeset 이 와도 7개 전부 같은 version. 근거는 [RELEASE_STRATEGY.md §1](./RELEASE_STRATEGY.md)
- **changesets 가 못 건드리는 3곳** — root `package.json`, platform 바이너리 9개(`packages/core-*`), `@zntc/core` 의 `optionalDependencies` 안 `@zntc/core-*` 버전. 앞의 둘은 `workspaces` 밖이라(platform 패키지를 workspaces 에 넣으면 `bun publish` 의 workspace→version 변환 semantics 가 바뀐다) changesets 의 시야에 없고, 셋째는 그래서 internal-dep 갱신 대상이 아니다.

  `bun run changeset:version` 이 `scripts/sync-release-versions.ts` 를 자동으로 돌려 셋 다 맞춘다 (진실의 원천 = `packages/core` 의 version). 손으로 올릴 필요 없다.

  > 이 셋 중 하나라도 빠지면 **`0.1.3` core 가 `0.1.2` 네이티브 바이너리를 받는다** — `optionalDependencies` 가 옛 버전을 가리키기 때문. v0.1.2 / v0.1.3 까지는 수동으로 올렸다.
