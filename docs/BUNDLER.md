...
src/bundler/
  ...
  │   출력: 절대 파일 경로 (+ 알려진 query suffix 재부착)
  │   책임: node_modules, package.json, tsconfig paths, query-suffix strip/재부착
  │   의존: 파일시스템만. 파서 불필요.
  │
  ├─ graph.zig            # 모듈 그래프
  │   입력: 진입점 목록
...
`import txt from "./a.txt?raw"` 처럼 specifier 뒤에 붙는 query 로 **import 단위** 로딩 방식을
고르는 관용구. Vite 생태계 문서·레시피가 널리 쓰는데 zntc 가 resolve 하지 못해
`ZNTC0100 Cannot resolve module` 이 났다 (monaco 의 워커 셋업이 `?worker` 를 쓴다).

```
"./icon.png?url"
        ↓  (resolver: 파일시스템을 치기 전에 알려진 query 를 뗀다)
"./icon.png"  → 일반 resolve (node_modules / extensions / tsconfig paths)
        ↓  (resolve 된 절대경로에 query 를 다시 붙인다)
"/abs/icon.png?url"   ← 모듈 경로 = dedup 키
        ↓  (loaders: query → 로더 강제, loader_explicit 로 마킹)
asset Module (loader = .file, inline-limit 무시)
```

- **query 를 경로에 유지하는 이유**: 같은 파일도 query 마다 **다른 모듈**이어야 한다
  (`x.png` 는 자산, `x.png?raw` 는 문자열). 모듈 경로가 dedup 키라, query 를 떼버리면 둘이
  한 모듈로 합쳐진다.
- **로더 매핑**: `?raw` → `.text`, `?url` → `.file`, `?inline` → `.dataurl`. 셋 다
  `loader_explicit` 로 마킹해 `--asset-inline-limit` 이 덮어쓰지 못하게 한다 — 사용자가
  URL 을 명시 요청했는데 data URL 이 나오면 요청과 어긋난다 (위 "기본 asset 로더 + inline
  limit" 의 "명시 요청이 암묵 기본값을 이긴다" 규칙과 같은 원리).
- **`?worker` / `?sharedworker`**: 새 인프라를 만들지 않고 **표준 worker 패턴을 합성**해
  기존 worker 기계를 그대로 재사용한다. 합성 소스가 `new Worker(new URL(...))` 표준 패턴이라
  import_scanner 가 worker record 로 잡고, 별도 청크로 빌드된 뒤 URL 이 최종 파일명으로
  재작성된다.
...
- **`Module.diskPath()`**: `path` 는 query 를 유지하므로 파일 열기 / basename / 확장자 계산에는
  query 를 뗀 경로를 써야 한다. 이걸 빼먹으면 `icon.png?url` 의 확장자가 `.png?url` 이 된다.
- **알려지지 않은 query 는 건드리지 않는다** — `?vue&type=style&lang.css` 는 플러그인이 가상
  경로로 처리하는 기존 관용구라 그대로 흘려보낸다. 알려진 토큰(`raw`/`url`/`inline`/`worker`/
  `sharedworker`)만 해석하며, `?worker&inline` 처럼 조합되면 **worker 가 이긴다** (Vite 동일).
