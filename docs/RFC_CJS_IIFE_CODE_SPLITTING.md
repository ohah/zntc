...
- **[PR3 한계, 문서화]** IIFE 정적 cross-chunk 는 dep 청크 `<script>` 가 entry 보다 먼저 평가돼야 함(`__zntc_require`가 동기) — 호스트가 dep 스크립트를 entry 앞에 배치하거나 동적 `import()`(로드 await)만 사용. self-installing register 로 load-order 요구는 "entry 마지막" 하나로 축소(RFC §6).
- **[검증됨 2026-05-16, RSC×factory]** `"use client"`/`"use server"` 디렉티브 × iife/umd-split: 기존 hoisted_directives 기계가 디렉티브를 모든 청크(entry·동적) byte 0(registry/factory/UMD prologue 위)에 호이스트 — **이미 정확, 구현 불요**. 회귀 테스트로 고정(iife+umd, 양 청크 byte 0 + 실행). PR4 후속 목록에서 제거.
- **[의도적 non-goal, preserve-modules+iife/umd/amd]** 가드가 `PreserveModulesRequiresESM` 로 거부(정답). preserve-modules 는 모듈 1:1 파일을 *소비자의 모듈 시스템*이 배선하는 라이브러리-저작 기능 — iife/umd/amd 는 per-file 모듈 시스템이 없어(P3-A 의 CJS 는 Node native require 로 가능했음) 개념상 무의미. esbuild 미지원, 실수요 0. splitting_dev `PreserveModules: IIFE ... PreserveModulesRequiresESM` 테스트가 이 의도를 고정. **구현 안 함(스코프 아웃).**
...
- **[수정됨 2026-05-16, PR4 CSP nonce]** strict CSP `script-src 'nonce-..'` 에서 동적 주입 `<script>` 차단. 호스트가 `globalThis.__zntc_nonce` 설정 → 로더 DOM 분기가 `s.setAttribute("nonce", g.__zntc_nonce)` (webpack `__webpack_nonce__` 대응). `__zntc_public_path` 와 동일 호스트-set 런타임 변수(빌드 옵션 불필요). importScripts/import() 분기는 `<script>` 비주입이라 무관. set/unset Node 실행 검증. helpers.ts `writeIifeDriver` 에 `nonce?` 파라미터 통합(dom 스텁 nonce-recorder, `{}` crash 도 동시 수정).
...
- **[수정됨 2026-05-16, 후속]** `export * from "./y"`(re_export_star, y 별도 청크) → 재-exporter 가 side-effect import 만 받아 inner 전체 export 미바인딩(link error). `collectExportsRecursive` 로 소스 effective export(nested/diamond 포함) 전부 열거, `linkReExportName`(named·star 공용 추출 헬퍼)로 canonical 다른 청크면 cross-chunk named 바인딩+재노출. esm/cjs Node 실행 검증. **`export * as ns from "./y"`(re_export_namespace, y 별도 청크) 및 `import * as P` cross-chunk 소비는 여전 미해결(깊은 linker 후속, 다른 re-export 버그들과 달리 bounded 아님).** 정밀 근본원인(2026-05-16 조사): 단일번들은 namespace **객체를 합성하지 않고** `ns.a`→`a` *멤버접근 elision*(심볼이 동일 스코프 hoisted 로컬일 때)으로 동작. cross-chunk 에선 ① re-exporter 청크에 소스 멤버 미바인딩(computeCrossChunkLinks 가 `.re_export_namespace` 미처리 — `.re_export_star` 패턴+소스모듈 collectExportsRecursive 로 부분 보완 가능) ② **그러나 핵심 갭은 importer 측**: `import {inner} from "./page"; inner.a` 에서 linker 가 `inner.a`→`a` 로 elide 하지만 computeCrossChunkLinks 가 *접근 모듈*의 청크에 `a` 를 cross-chunk 바인딩하지 않음 → bare `a` 미바인딩. 수정엔 linker 의 namespace-멤버-접근 resolution 결과를 computeCrossChunkLinks 가 소비해 접근 모듈 청크에 canonical 멤버를 등록하는 *깊은* seam 필요(getResolvedBinding/static-member resolution 연계). 광범위 회귀 위험으로 별도 집중 작업 권장. (RFC §7 추적, P3-B 5번째 re-export-family 버그 — 나머지 4개 #3350/#3354/#3351/#3353 는 bounded·수정 완료.)
