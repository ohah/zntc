...
| **React Native CLI + MCP** | S~M  | 부분 (preset+dev-server 존재, CLI 진입점만 부재) | `react-native.config.js` command plugin 으로 `start`/`bundle` 노출 (빌드는 코어 `.node` 유지, 신규 빌드 경로 아님) + 기존 dev-server MCP 에 RN 도구 추가. 아래 "RN CLI + MCP / Chrome CDP 계획" 참조 |
| **Chrome CDP 검증 + MCP/CLI** | L | 부분 (E2E 내부 전용) | CDP(Playwright, `tests/e2e/`)가 현재 내부 테스트 전용. 번들을 Chrome 에서 실행·검증·디버그하는 사용자 CLI/MCP 도구로 승격. 아래 계획 참조 |
| **RN Hermes 타겟 다운레벨** | M | 부분 (엔진 타겟 인프라 ✅, RN preset 미연결) | RN preset 이 `target:'es5'` 하드코딩 (`preset.ts:469`). 엔진 타겟(`compat.zig` `hermes` enum, Phase 16) 은 이미 완성 — Hermes 버전 feature-level 다운레벨로 매핑해 ES5 일괄 변환 bloat 제거 예정. 아래 "RN Hermes 타겟 다운레벨 계획" 참조 |
...
### RN CLI + MCP / Chrome CDP 계획

두 트랙 모두 **기존 인프라의 사용자-노출 승격**이지 신규 스택이 아니다. 특히 RN 빌드는 **이미 코어 `.node`(Zig) 가 100% 수행** — `packages/react-native/src/preset.ts:374` 의 `buildRnBundleOptions` 는 번들러가 아니라 `RnBundleInput → BuildOptions` **config 변환기**일 뿐이고, `--platform=react-native` 로 코어 CLI 가 이미 RN 번들을 만든다 (사용자 실측 확인). `packages/react-native/src/dev-server/` 역시 자체 번들링 없이 코어 `watch`/`WatchRebuildEvent` 를 Metro 패키저 프로토콜(`/index.bundle`·`/symbolicate`·HMR ws)로 어댑트하는 서버로 **이미 존재**. 즉 RN 트랙에서 새로 필요한 건 빌드 코드가 아니라 `react-native.config.js` 가 발견하는 **command plugin 진입점 하나**(argv → 기존 `buildRnBundleOptions`/dev-server 배선)뿐. (1) RN deps: `packages/react-native/` 가 `@react-native-community/cli-server-api` 를 `dependencies` 로, `metro-resolver`·`@react-native/babel-preset` 을 `optionalDependencies` 로 이미 보유. (2) dev-server MCP: Zig 네이티브 서버에 JSON-RPC 2.0 (`initialize`/`tools/list`/`tools/call`, 도구 `reset_cache`·`get_build_events`) 이미 구현 (`src/server/dev_server.zig:1003`). (3) CDP: Playwright 기반 Chromium E2E (`tests/e2e/` — sourcemap·smoke·code-splitting-runtime 등) 가 이미 동작하나 **내부 테스트 전용**.

| 단계 | 항목 | 난이도 | 선행 | 설명 |
|---|---|---|---|---|
| RC-1 | **RN CLI command plugin (argv 어댑터)** | S~M | preset+dev-server ✅ | `@zntc/react-native` 에 `react-native.config.js` 의 `commands` 플러그인 진입점 추가 — `bundle`/`start` argv → 기존 `buildRnBundleOptions` (config 변환) → 코어 `.node` `build()`/`watch()`, `start` 는 기존 `src/dev-server/serve.ts` 기동. **빌드 코드 재구현 없음** (코어 `.node` 가 이미 RN 빌드 수행). 사용자는 플러그인만 추가 → 기존 `npx react-native start/bundle` 이 그대로 ZNTC 경유. 코어 `zntc` CLI 는 RN deps 미오염(경계 유지) |
| RC-2 | **dev-server MCP 에 RN 도구 추가** | M | RC-1 | 기존 `tools/list` 에 `rn_bundle`(플랫폼/dev 인자) · `rn_reload`(앱 새로고침) 추가. `get_build_events` 는 그대로 RN 빌드 이벤트도 커버. LLM 에이전트가 RN 빌드/리로드를 `tools/call` 로 구동 |
| CD-1 | **CDP 번들 검증 CLI** | M | — | E2E 전용 CDP 경로를 `zntc verify --browser` (가칭) 사용자 명령으로 승격 — 번들을 헤드리스 Chrome 에서 실행, console error/uncaught/sourcemap 해석 결과를 종료코드+JSON 으로 리포트. Playwright 의존은 선택적(optionalDependency) |
| CD-2 | **CDP 검증을 MCP 도구로 노출** | M | CD-1, RC-2 | dev-server MCP 에 `verify_in_chrome` 도구 추가 — 에이전트가 빌드 후 브라우저 런타임 검증까지 한 루프로 (빌드→Chrome 실행→런타임 에러 수집). CD-1 코어 재사용 |

**범위 결정**: 네 항목 모두 **신규 스택 도입이 아니라 이미 있는 것(RN preset / dev-server / dev-server MCP / Playwright-CDP)의 사용자 노출**이라 측정 리스크가 낮다. RC-1 은 당초 "L / RN CLI 호환 구현" 으로 무겁게 잡혔으나, 실측 결과 빌드는 코어 `.node` 가 100% 수행하고 `buildRnBundleOptions`(config 변환)·`dev-server/serve.ts` 가 이미 존재 → **argv 어댑터 + command plugin 진입점 한 개(S~M)** 로 재평가. RN deps(`cli-server-api` 등) 가 코어로 새지 않도록 진입점은 `@zntc/react-native` 에 두는 게 핵심(코어 `zntc` 는 범용 유지). CD-1(CDP CLI)은 RN 과 독립이라 병렬 가능. 우선순위는 1~2단계(안정성/CSS) 뒤, 3단계 생태계 — RN 트랙(NativeWind·RC)과 에이전트/툴링 트랙(CD)으로 갈래. Playwright 는 항상 optionalDependency 로만 (코어 의존 금지).

### RN Hermes 타겟 다운레벨 계획

현재 RN preset 은 `target: 'es5'` 를 **무조건 하드코딩** (`packages/react-native/src/preset.ts:469`, 주석 `:4` "RN platform 시 target=es5"). 그러나 엔진 타겟 인프라는 이미 완성 — `src/transformer/compat.zig` 의 `Engine` enum 에 `hermes` 가 있고 per-feature 표가 `hermes 0.7 = 0/16 미지원, hermes 0.12 = 16/16` 까지 compat-table.github.io 로 검증돼 있음 (Phase 16). 즉 갭은 인프라가 아니라 **RN preset 이 이 엔진 타겟을 안 쓰고 ES5 로 일괄 다운레벨하는 것**. 최신 Hermes(0.12+, 현행 RN 동봉)는 class/let/arrow/destructuring 등 16개 ES2015+ feature 를 네이티브 지원하므로 ES5 일괄 변환은 helper/클로저 bloat 만 키운다.

| 단계 | 항목 | 난이도 | 선행 | 설명 |
|---|---|---|---|---|
| HD-1 | **RN preset 을 Hermes 엔진 타겟에 매핑** | M | 엔진 타겟 ✅ | 하드코딩 `target:'es5'` → RN 버전이 동봉한 Hermes 버전 기반 `--target=hermes<ver>` feature-level 다운레벨. RN 버전 → Hermes 버전 매핑 테이블 필요 (RN 0.70+ Hermes 0.12 라인). 사용자 override 시 우선 |
| HD-2 | **Metro 호환 출력 재검증** | M | HD-1 | 네이티브 class/let 출력이 `__d` wrap·scope-hoist·기존 Hermes 워크어라운드(`preset.ts` var-redeclare / iOS 26.4 IIFE wrap)와 충돌 없는지 RN smoke + E2E 회귀 가드. Hermes 미지원 잔여(named capture regex 등 — GitHub Issues 의 Hermes 미해결 항목)는 그대로 다운레벨 유지 |
| HD-3 | **`hermesEnabled:false` (JSC) 폴백** | S | HD-1 | Hermes 비활성 RN 앱(구형/JSC)은 ES5 유지. `react-native.config.js`/preset 입력으로 분기 |

**범위 결정**: 인프라(compat.zig 엔진 타겟)는 이미 있어 신규 스택 아님 — 핵심은 RN preset 의 기본값을 ES5 고정에서 Hermes feature-level 로 바꾸는 **정확성 민감 변경**(M). 번들 사이즈/런타임 이득은 measure-first 로 검증 후 기본값 전환 (RN 런타임·Metro·Hermes 버전 매트릭스가 회귀 표면). HD-1 은 RC-1 command plugin 과 같은 RN 트랙이라 그 뒤 연계.
...
**BoringSSL pin 갱신 cadence**: BoringSSL 은 공식 "live at head" 정책 (tagged release 비권장 — `INCORPORATING.md`). ZTS 는 **Chromium stable channel** 의 `boringssl_revision` 을 따라간다 (~6주 cycle, Chrome major release 와 함께). **manual cadence** — `.github/dependabot.yml` 의 `gitsubmodule` ecosystem 은 BoringSSL 을 ignore 한다 (upstream HEAD auto-bump 가 Chromium stable channel 과 충돌해 검증되지 않은 commit 으로 PR 이 올라옴 — hotfix #3804). 갱신 흐름: Chromium release 노트에서 새 `boringssl_revision` 확인 → `git submodule update vendor/boringssl --remote` 로 fetch + `git checkout <commit>` 으로 pin → 회귀 test → PR.

**한계**: dependabot 의 `gitsubmodule` ecosystem 은 어차피 보안 advisory 자동 PR (security update) 미지원 — manifest 기반 (npm/pypi/등) 만 advisory DB 매칭됨. 따라서 BoringSSL CVE/security advisory 즉시 인지는 별도 subscribe (BoringSSL upstream announce, Chromium security advisories) 가 필요하며, advisory 발생 시 manual 즉시 bump. Bun fork (oven-sh/boringssl) 는 채택하지 않는다 (Node `crypto` API 호환 patch 가 ZTS scope 와 무관).
