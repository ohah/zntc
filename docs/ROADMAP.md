...
esbuild는 `NoSideEffects_PureData` 마킹으로 이를 해결하지만, ZNTC의 tree-shaker는 CJS wrap에 대해 아직 이 최적화를 수행하지 않음.
...
| 항목                             | 난이도 | 현재 상태 | 설명                                                                                                                                                                                                                         |
| -------------------------------- | ------ | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **통합 테스트 확대**             | L      | 부분 완료 | ✅ Vite: multi-module TS/TSX 앱을 `vite-plugin-zntc`로 프로덕션 번들 → Playwright 브라우저 실행 E2E (`tests/e2e/tests/vite-app-e2e.test.ts`). **Remix/Next.js는 3단계(프레임워크 통합)로 이동** — 아래 "1단계 범위 결정" 참조 |
...
**Vite** — 번들러 교체 가능 구조. Vite 코어가 dev server / HMR / plugin API를 제공하고 TS/JSX 변환은 esbuild에 위임. `vite-plugin-zntc`가 esbuild 자리에 들어감. 플러그인 하나로 통합.
...
| **npm 배포**         | L      | 로컬 빌드만                                            | `npm install zntc`로 설치. cross-platform prebuilt binary (macOS/Linux/Windows arm64/x64) |
...
| ~~**vite-plugin-zntc**~~  | ✅     | 완료      | Vite의 esbuild transform을 ZNTC로 교체하는 플러그인                                                                                                                                                                          |
