...
| Phase                       | 내용                                                                                                                                                                                                                                                                                                       | 상태 |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| 1. 렉서                     | 토큰 142개, SIMD, 유니코드, JSX, RegExp 검증                                                                                                                                                                                                                                                              | ✅   |
...
- ✅ 코드 스플리팅 시 청크별 CSS 분리 emit (도달 모든 청크에 복제 = esbuild/webpack 동치, 동적 청크 런타임 `<link>` 주입)
- 후순위: core `--bundle` 모드의 CSS Modules (`.module.css`) 자동 변환 (앱 모드는 빌트인 지원)
...
- ~~**CSS 번들링**~~ — ✅ Phase 1 완료
  ...
  코드 스플리팅 시 청크별 CSS 분리 emit ✅ (도달 모든 청크에 복제, 동적 청크 런타임 `<link>`).
  후순위: core `--bundle` 모드의 CSS Modules (`.module.css`) 자동 변환 (앱 모드는 빌트인).
...
| **React Native CLI** | S~M  | 부분 (preset+dev-server 존재, CLI 진입점만 부재) | `react-native.config.js` command plugin 으로 `start`/`bundle` 노출 (빌드는 코어 `.node` 유지, 신규 빌드 경로 아님). 아래 "RN CLI / Chrome CDP 계획" 참조 |
| **Chrome CDP 검증 CLI** | L | 부분 (E2E 내부 전용 + `zntc verify` CLI) | CDP(Playwright, `tests/e2e/`)가 현재 내부 테스트 전용 + `zntc verify --browser` 사용자 CLI 로 일부 승격됨. 추가 디버그 도구로 확장 가능. 아래 계획 참조 |
...
### RN CLI / Chrome CDP 계획

두 트랙 모두 **기존 인프라의 사용자-노출 승격**이지 신규 스택이 아니다. 특히 RN 빌드는 **이미 코어 `.node`(Zig) 가 100% 수행** — `packages/react-native/src/preset.ts` 의 `buildRnBundleOptions` 는 번들러가 아니라 `RnBundleInput → BuildOptions` **config 변환기**일 뿐이고, `--platform=react-native` 로 코어 CLI 가 이미 RN 번들을 만든다 (사용자 실측 확인). `packages/react-native/src/dev-server/` 역시 자체 번들링 없이 코어 `watch`/`WatchRebuildEvent` 를 Metro 패키저 프로토콜(`/index.bundle`·`/symbolicate`·HMR ws)로 어댑트하는 서버로 **이미 존재**. 즉 RN 트랙에서 새로 필요한 건 빌드 코드가 아니라 `react-native.config.js` 가 발견하는 **command plugin 진입점 하나**(argv → 기존 `buildRnBundleOptions`/dev-server 배선)뿐. (1) RN deps: `packages/react-native/` 가 `@react-native-community/cli-server-api` 를 `dependencies` 로, `metro-resolver`·`@react-native/babel-preset` 을 `optionalDependencies` 로 이미 보유. (2) CDP: Playwright 기반 Chromium E2E (`tests/e2e/` — sourcemap·smoke·code-splitting-runtime 등) 가 이미 동작 + `zntc verify --browser` 사용자 CLI 로 일부 승격됨.
...
| CD-1 | **CDP 번들 검증 CLI 확장** | M | `zntc verify --browser` ✅ | 현재 verify 는 console error/uncaught/sourcemap 해석을 종료코드+JSON 으로 리포트. 추가 디버그 도구 (네트워크 캡처, 스냅샷 등) 로 점진 확장 가능. Playwright 의존은 항상 선택적(optionalDependency) |

**범위 결정**: 두 항목 모두 **신규 스택 도입이 아니라 이미 있는 것(RN preset / dev-server / Playwright-CDP)의 사용자 노출**이라 측정 리스크가 낮다. RC-1 은 당초 "L / RN CLI 호환 구현" 으로 무겁게 잡혔으나, 실측 결과 빌드는 코어 `.node` 가 100% 수행하고 `buildRnBundleOptions`(config 변환)·`dev-server/serve.ts` 가 이미 존재 → **argv 어댑터 + command plugin 진입점 한 개(S~M)** 로 재평가. RN deps(`cli-server-api` 등) 가 코어로 새지 않도록 진입점은 `@zntc/react-native` 에 두는 게 핵심(코어 `zntc` 는 범용 유지). CD-1(CDP CLI)은 RN 과 독립이라 병렬 가능. 우선순위는 1~2단계(안정성/CSS) 뒤, 3단계 생태계 — RN 트랙(NativeWind·RC)과 에이전트/툴링 트랙(CD)으로 갈래. Playwright 는 항상 optionalDependency 로만 (코어 의존 금지).
