...
두 트랙 모두 **기존 인프라의 사용자-노출 승격**이지 신규 스택이 아니다. 특히 RN 빌드는 **이미 코어 `.node`(Zig) 가 100% 수행** — `packages/react-native/preset.ts:374` 의 `buildRnBundleOptions` 는 번들러가 아니라 `RnBundleInput → BuildOptions` **config 변환기**일 뿐이고, `--platform=react-native` 로 코어 CLI 가 이미 RN 번들을 만든다 (사용자 실측 확인). `packages/react-native/dev-server/` 역시 자체 번들링 없이 코어 `watch`/`WatchRebuildEvent` 를 Metro 패키저 프로토콜(`/index.bundle`·`/symbolicate`·HMR ws)로 어댑트하는 서버로 **이미 존재**. 즉 RN 트랙에서 새로 필요한 건 빌드 코드가 아니라 `react-native.config.js` 가 발견하는 **command plugin 진입점 하나**(argv → 기존 `buildRnBundleOptions`/dev-server 배선)뿐. (1) RN deps: `packages/react-native/` 가 이미 `@react-native-community/cli-server-api` · `metro-resolver` · `@react-native/babel-preset` 의존. (2) dev-server MCP: Zig 네이티브 서버에 JSON-RPC 2.0 (`initialize`/`tools/list`/`tools/call`, 도구 `reset_cache`·`get_build_events`) 이미 구현 (`src/server/dev_server.zig:1003`). (3) CDP: Playwright 기반 Chromium E2E (`tests/e2e/` — sourcemap·smoke·code-splitting-runtime 등) 가 이미 동작하나 **내부 테스트 전용**.

| 단계 | 항목 | 난이도 | 선행 | 설명 |
|---|---|---|---|---|
| RC-1 | **RN CLI command plugin (argv 어댑터)** | S~M | preset+dev-server ✅ | `@zntc/react-native` 에 `react-native.config.js` 의 `commands` 플러그인 진입점 추가 — `bundle`/`start` argv → 기존 `buildRnBundleOptions` (config 변환) → 코어 `.node` `build()`/`watch()`, `start` 는 기존 `dev-server/serve.ts` 기동. **빌드 코드 재구현 없음** (코어 `.node` 가 이미 RN 빌드 수행). 사용자는 플러그인만 추가 → 기존 `npx react-native start/bundle` 이 그대로 ZNTC 경유. 코어 `zntc` CLI 는 RN deps 미오염(경계 유지) |
