...
| **React Native CLI + MCP** | L    | 부분 (RN preset / dev-server MCP) | `@react-native-community/cli` 호환 `start`/`bundle` 로 Metro 대체 + 기존 dev-server MCP 에 RN 빌드 제어 도구 추가. 아래 "RN CLI + MCP / Chrome CDP 계획" 참조 |
...
두 트랙 모두 **기존 인프라의 사용자-노출 승격**이지 신규 스택이 아니다. (1) RN: `packages/react-native/` 가 이미 `@react-native-community/cli-server-api` · `metro-resolver` · `@react-native/babel-preset` 의존 + `--platform=react-native` 프리셋 보유. (2) dev-server MCP: Zig 네이티브 서버에 JSON-RPC 2.0 (`initialize`/`tools/list`/`tools/call`, 도구 `reset_cache`·`get_build_events`) 이미 구현 (`src/server/dev_server.zig:1003`). (3) CDP: Playwright 기반 Chromium E2E (`tests/e2e/` — sourcemap·smoke·code-splitting-runtime 등) 가 이미 동작하나 **내부 테스트 전용**.

| 단계 | 항목 | 난이도 | 선행 | 설명 |
|---|---|---|---|---|
| RC-1 | **RN CLI 호환 `bundle`/`start`** | L | RN preset ✅ | `@react-native-community/cli` 의 `bundle`(프로덕션 JS bundle + assets)·`start`(Metro dev server) 인터페이스를 ZNTC 가 제공 — RN 프로젝트가 `metro.config.js` 대신 ZNTC 로 swap. Metro 호환 출력 (이미 RN smoke 검증) 재사용 |
...
**범위 결정**: 네 항목 모두 **신규 스택 도입이 아니라 이미 있는 것(RN preset / dev-server MCP / Playwright-CDP)의 사용자 노출**이라 측정 리스크가 낮다. RC-1(RN CLI 호환)이 의존 표면이 가장 커 선행. CD-1(CDP CLI)은 RN 과 독립이라 병렬 가능. 우선순위는 1~2단계(안정성/CSS) 뒤, 3단계 생태계 — RN 트랙(NativeWind·RC)과 에이전트/툴링 트랙(CD)으로 갈래. Playwright 는 항상 optionalDependency 로만 (코어 의존 금지).
