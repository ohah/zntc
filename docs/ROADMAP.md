...
**BoringSSL pin 갱신 cadence**: BoringSSL 은 공식 "live at head" 정책 (tagged release 비권장 — `INCORPORATING.md`). ZTS 는 **Chromium stable channel** 의 `boringssl_revision` 을 따라간다 (~6주 cycle, Chrome major release 와 함께). 자동화는 `.github/dependabot.yml` 의 `gitsubmodule` ecosystem 이 weekly 로 새 commit 후보를 PR 로 제안하며, 사람이 Chromium stable channel 과의 정합을 확인한 뒤 머지.

**한계**: dependabot 의 `gitsubmodule` ecosystem 은 보안 advisory 자동 PR (security update) 미지원 — manifest 기반 (npm/pypi/등) 만 advisory DB 매칭됨. 따라서 BoringSSL CVE/security advisory 즉시 인지는 별도 subscribe (BoringSSL upstream announce, Chromium security advisories) 가 필요하며, advisory 발생 시 weekly cadence 무시하고 manual 즉시 bump. Bun fork (oven-sh/boringssl) 는 채택하지 않는다 (Node `crypto` API 호환 patch 가 ZTS scope 와 무관).
