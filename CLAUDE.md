...
## Documentation
- [docs/USAGE.md](./docs/USAGE.md) — CLI 실행법, `@zntc/core` JS API, 플랫폼/watch/ES 타겟/CSS 동작 포인트
- [docs/CONFIG.md](./docs/CONFIG.md) — `zntc.config.{ts,js,json}` / `tsconfig` / `.env` / CLI flag 우선순위·머지·함수형 config·conflict 예시
...
4. `gh pr create` 시 `--label`, `--assignee` 항상 지정
5. PR 제목은 `feat(lexer): add numeric literal tokenization` 형식
6. PR 본문, GitHub 이슈/PR 댓글, 리뷰 답변은 한국어로 작성
7. Zig 초보자에게 설명 포함
